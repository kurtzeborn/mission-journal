...
**Partly shipped.** Accumulation landed early in Phase 6 — the `pending` container exists, an unclaimed direct send is held there, and `claim.json` carries the rolling window and message count. **The whole claim half remains:** HMAC token signing, the claim email, the `/claim/{token}` page, promotion into `raw/`, dedup, the reminder, and the expiry purge. Nothing yet reads `pending/`, so letters accumulate and wait.
...
- **`pdayletters.com` must be onboarded for *sending*, not just routing.** Until it is, mail can only go to the account's own verified destination addresses — fine for testing, useless for a claim email, which by definition goes to a stranger. Sending records live on a `cf-bounce.` subdomain with their own DKIM selector, entirely separate from the routing records on the root, so onboarding does not disturb inbound mail. The one collision is `_dmarc`, which already exists at `p=none`.
