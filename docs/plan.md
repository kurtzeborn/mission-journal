...
- **`@missionary.org` runs Google Workspace behind Proofpoint, with DMARC at `p=quarantine`.** Verified by DNS lookup, July 2026:
  ...
  This largely **de-risks the `direct` path**: an enforcing `p=quarantine` policy can only be maintained if the domain's own outbound mail aligns, so `Authentication-Results` on a genuine missionary email should show a DMARC pass.

  It also clarifies **where filtering happens**. Proofpoint appears only in SPF, so it is the **outbound relay** — inbound mail to a missionary lands at **Gmail**, and Gmail's filtering is what our replies must satisfy. Gmail weights prior correspondence with a sender heavily, and its behavior can be tested against any ordinary Gmail account. (A tenant can route inbound through a third-party scanner via Google-side rules that DNS won't reveal, so this is the likely picture rather than a certain one.)
...
Three of the constraints above cannot be verified until someone with a real `@missionary.org` account is available. That access is expected partway through the build, so the work is **sequenced behind everything else**, and built so that being wrong is cheap and immediately visible.

**The blind parts come last.** [Stage 1](#stage-1--vertical-slice) is forward-only: it needs a genuine missionary letter, but only as an *attachment* someone forwards, which any family member on a mission already has. Everything that requires *sending* from `@missionary.org` — `direct` classification, `claim@`, the ownership flow — is deferred to [Phase 6](#phase-6--direct-ingest) and later.

**The missionary domain is configuration, not a constant.** A `MISSIONARY_DOMAINS` Function app setting (default `missionary.org`) is what the classifier, slug derivation, and the `claim@` handler check against. Pointing it at a domain we control makes the entire `direct` and `claim@` flow testable end to end, including DMARC evaluation, slug derivation, claim-token issuance, sign-in binding, and the `verifiedMissionary` ACL write. It stays worth keeping afterwards, because it is also how the service absorbs a second Church domain.

**Most of the risk isn't logic — it's header format.** Our code never performs DMARC itself; the inbound provider does, and the classifier reads the resulting `Authentication-Results` header. Classification *logic* is fully testable offline against hand-crafted `.eml` fixtures. The unknown is the exact shape of that header for real `missionary.org` mail — a parsing question with a narrow failure surface, not an architectural unknown.

**Instrument for the first real message**, since it is the actual test:
...
- `inbox/` retains the verbatim payload for 30 days, so a message misclassified by a header-parsing bug can be reprocessed after a fix rather than lost.

**Feature-flag `claim@`.** Until enabled it accepts mail and does nothing, which is indistinguishable from the documented "ignored without reply" behavior and therefore leaks nothing.

**The first real missionary is the pilot**, and should be someone we can talk to directly, so a failure is a conversation rather than a support ticket from a stranger.
...
Cloudflare Email Routing accepts a catch-all, so *every* address at the domain arrives at the Worker. The classifier honors exactly these two local-parts and silently drops the rest; there is no per-address route configuration to keep in sync.
...
**MX lives on the apex**, pointing at the inbound provider. Outbound does not collide with it: bounce handling lives on `cf-bounce.pdayletters.com` and never touches the inbound parse path.
...
**Outbound mail is DKIM-signed as `pdayletters.com`.** The visible `From:` differs by message type: **replies to inbound mail are sent from the address the sender wrote to** (`post@` or `claim@`) to preserve the recipient's prior-correspondence signal, while self-originated mail — reminders, ownership nudges, and [invitations](#invitations) — comes from `hello@pdayletters.com`, which routes to a human. DMARC passes either way through DKIM alignment.
...
This is a rough edge with no good automated fix: we cannot publish DNS on someone else's domain, and relaxing the check to accept bare SPF would weaken the one authentication signal the forward path has. What the service *can* do is say so in the admin UI at invite time, rather than letting an invited member discover it by forwarding twenty letters into a void. Deferred until the invitation flow exists (Phase 7).
...
Re-verification works, and it fails far more often than the design first assumed. **[docs/dkim-findings.md](dkim-findings.md) carries the measurements** — per-client results, the Exchange byte-level diff, the adversarial ARC matrix, and the prior art. The conclusions that shape the code:

- **`bh=` and `b=` are checked separately.** RFC 6376 makes them independent, and `mailauth` short-circuits on a body-hash mismatch without ever checking the signature — correct for a delivery decision, useless for this one. Splitting them is what recovered the Outlook web path.
- **A trusted ARC seal covers the body when the hash is gone**, and only `microsoft.com` is trusted, because it is the one provider whose store rewrites bodies. Seal and header signature are both required: the seal does not cover `From:`, and `b=` says nothing about the body.
- **`coverage: 'body' | 'headers' | null`** rides alongside `verified`. `body` means the published words are the signed words; `headers` means only that the letter is from the address and date it claims.
- **The clock is the message's earliest `t=`, not today.** Google expires signatures about a week out, and an archive re-verifies letters months or years later, so a wall-clock check always answers "expired". Earliest, not latest, or the forwarder's own re-signing moves the clock past the missionary's expiry. This gives up replay protection deliberately — a replay here is a letter the missionary genuinely did sign.
- **Desktop and Android Outlook forwards are unrecoverable.** Desktop rebuilds the message from MAPI properties, altering four signed headers and truncating the ARC chain; Android sends no `DKIM-Signature` at all. That is what forced the fallback below.
...
There are two such replies — one for a letter quoted inline, one for a letter attached that did not verify — and **both carry the relay link as their second route**. The author address on an inline forward is text the sender typed; the author address on an unverified attachment is a header in a file the sender wrote. Neither is evidence, both take seconds to forge, and the fences behind the link are identical. Withholding it from the inline reply would deny the route to whoever has the worst mail client — the Outlook and Gmail phone apps have no *forward as attachment* item at all.
...
**It also puts the decision where it belongs.** The question at bootstrap is *should this person be allowed to start an archive of your letters*, and a signature only ever answered it by proxy, on the assumption that anybody holding a genuine letter was already trusted with it. Here the person whose letters they are answers it directly.
...
**The oracle this opens, stated plainly.** Triggering the note requires a signed link, and that link is minted for the sender of anything that *looked* like a missionary's letter and could not be authenticated — an attached original that failed to verify, or quoted text naming an author. Both are by definition unauthenticatable, so a fabricated message reaches either. Somebody can therefore cause one message to be sent to a plausible `@missionary.org` address, naming themselves. It is rate-limited to one outstanding grant per missionary and it is not silent: the note names the requester twice, once to act on and once to refuse on, and says outright what forwarding the link hands over. The missionary is the control.
...
```jsonc
{
  ...
  "photos": [
    ...
    // Added by an owner rather than attached to the letter. `addedAt` is what
    // says so; `takenAt` is the capture time read in the browser, absent when
    // neither the file's metadata nor its name gave one.
...
**Fetching a Google Photos album is not possible, at any price** (verified August 2026). The obvious remedy — give the service its own Google account and have families share the album with it — cannot work:
...
**Photos is not a view onto Drive, so Drive access does not reach a Photos album.** The two products cross-synced until Google separated them on 10 July 2019. Three things establish the separation:
...
- **Model:** per-missionary allowlist, matched on the sign-in first and the email address second.
- **Two keys, because an address is not a person.** Google treats `jane.doe@gmail.com` and `janedoe@gmail.com` as one account but hands back only the spelling it was registered with, and a Microsoft account's primary address can change under an entry that keeps pointing at the old one. Either way the invited person signs in successfully and then cannot see the archive, which reads as the service being broken rather than as a typo. So an ACL entry now also carries `identity` — `provider:userId`, both halves, because Static Web Apps mints `userId` per provider — and `resolveAccess` tries that before the address. **The identity is an additional key, not a replacement:** an entry matches on either, so binding one can never take access away from somebody who already had it. Refusing the address once an identity is bound would harden the list against a recycled mailbox, which is remote; it would also lock out the same human arriving through their other provider, which is not. **Operators are deliberately left out of this** — they are matched on address alone, so the same person is the same operator through Google or Microsoft.
- **The stamp is applied at sign-in, and a changed address is detected there too.** The `identities` table holds one row per identity — `RowKey` is a SHA-256 of `provider:userId` — carrying the last address that identity used. The `/api/memberships` call is the nearest thing this API has to a sign-in hook, so reconciliation happens there: an address that matches the stored one costs a single point read and stops, a first sign-in stamps the identity onto every ACL the address is already on, and an address that has *changed* moves both the ACL entries and the membership index rows across. The table is derived, and the repair for losing it is somebody signing in again. Shipped for [issue #13](https://github.com/kurtzeborn/mission-journal/issues/13).
...
**An operator is an owner on every slug, resolved at authorization time.** The shared ACL check behind [Private content delivery](#private-content-delivery) returns `owner` for an operator on any site, and **nothing is written to that site's `acl.json` or to `memberships`**. So operators never appear in an owner's invitee list, never populate a site switcher or the signed-in root redirect (an operator reaches a site by typing its URL), and the owner admin UI needs no operator-specific variant. Adding or removing an operator is one config change instead of a fan-out write across every site.

**The operator list is configuration, not data** — an `OPERATOR_EMAILS` Function app setting holding one or two addresses. A privilege this broad must not be grantable through the interface it grants: if operators lived in a blob or a table, one compromised operator account could quietly add a second and make the escalation self-sustaining. As a setting, adding an operator requires Azure control-plane access, separately recorded in the Activity Log. There is deliberately no UI for editing it.
...
- **Reprocess raw mail service-wide.** `raw/` is preserved so history can be re-rendered after a sanitizer or extractor fix, and `inbox/` retains misclassified messages for 30 days so they can be re-run after a classifier fix (see [Building blind](#building-blind)). Owners can re-render a single post on their own site — see [Restoring the original](#restoring-the-original) — but a sweep across every slug is not an owner-shaped action, and `inbox/` belongs to no site at all.
...
- **Every operator action against a site they don't belong to emits an `OperatorAction` event** to App Insights — actor, slug, action, timestamp, and the reason string where one was collected. **Reads are logged, not only writes**, because reading a family's letters is the privilege that matters most here.
- **A persistent banner appears on any site where the operator is not an ACL member:** *"You are viewing this site as an operator. This access is logged."* This guards against the likelier failure — an operator forgetting which hat they are wearing and editing a real family's post.

Disclosing this access to owners in plain language belongs with the terms-of-use work in [Phase 12](#phase-12--leaving-beta).
...
- **Invitations carry the one-click opt-out**, which here means "never invite me to anything again", is global rather than per archive, and is honored ahead of any future invitation to that address.
...
So the answer is not a longer session, it is making the return trip invisible. Three flows, because expiry surfaces in three different places:
...
- **A tab left open.** The document and letter text remain on a phone after the edge session has expired, but photographs are separate authenticated requests and fail as they become visible. An image error does not expose its HTTP status, so treating every one as expiry would turn a genuinely missing picture into a sign-in loop. The website instead makes one shared, uncached `posts.json` probe; only a `401` or SWA's opaque redirect goes through `login.html`, where the remembered provider makes the return silent. A healthy response leaves an ordinary image failure alone, and a network failure leaves the tab in place because login cannot repair an offline phone.
...
**One click became none, by remembering which of the two.** Priced 2026-09-10 and it is not a session question at all: the cookie's length is still unpublished and still has no knob, but Google's and Microsoft's own sessions run for months and both registrations have already been consented to, so a trip out to whichever was used last returns without asking anything. The only thing missing was the name of the door. `login.js` records it on the button press — the last honest signal, since a sign-in that then failed was still attempted with that provider — and a later arrival replaces straight through to it. Three things keep it from being clever rather than safe: **one silent attempt per destination**, held in `sessionStorage`, because a trip that comes back still anonymous meets the same `401` and would otherwise loop; **`replace` rather than `assign`**, so Back from the letter cannot land on a page whose only behaviour is to throw them forward again; and **signing out means it**, which is why every sign-out link now returns through `/login.html?signedout=1` rather than to where it stood — returned to the page itself, the `401` would be answered silently with the very account just walked away from. An unrecognised remembered name is read as nothing remembered, because it comes back out of a store the visitor can edit. What this does not do is extend anything; it stops asking a question we can answer ourselves. The cost, said plainly: on a shared computer the next person at that keyboard reaches the archive without a prompt for as long as the browser profile holds the provider's session.
...
5. **Claim.** The link opens `pdayletters.com/claim#{token}`, and that page is **anonymous-allowed**: it leads with what is being claimed — the missionary's name, the counts, up to three subject lines and dates — then a plain statement of what happens next, then the **Sign in with Google / Sign in with Microsoft** pair.

   On return the Function validates the token, writes the signed-in identity into a new `config/{slug}/acl.json` as the first `owner`, and asks for **one setup field: the missionary's display name**, pre-filled from the `From:` display name on the letters already in hand. Submitting lands them on the site itself; a large backlog may still be rendering, and the site fills in as renders complete.

   **Expired and already-used links.** The token shares the pending site's rolling `expiresAt`, so it lives exactly as long as there is something to claim. The failure page still does something useful: if the site is now active, *"This site is already set up"* plus a sign-in link; if the pending site has purged, say so plainly; if the token is merely stale or spent, offer **"Email me a new link"**, which sends only to an address already listed in `claim.json`. **Shipped 2026-08-06** as `POST /api/claim/resend` — see [Asking for a new link](#asking-for-a-new-link).
6. On claim, everything accumulated moves to `raw/{slug}/` and is enqueued for render **in `originalDate` order, running dedup for the first time** — so a bulk dump comes out deduplicated and chronological in a single pass, with no interim half-built site for anyone to see. **Promotion suppresses acks**, published and deduped alike: the claimant is looking at those letters on screen, and mail telling them so would answer a question the site has already answered.
7. **Reminder and purge.** One reminder per pending site goes to the forwarder addresses already emailed, seven days after the *first* claim email for that site — keyed to the site, not to each individual claim email, so three forwarders don't produce three reminders on three clocks. **It reports what's waiting**, which is also the only confirmation a bulk forwarder ever gets that anything past their first message arrived. When `expiresAt` passes with no intervening message, a nightly timer purges `pending/{slug}/` entirely, including soft-deleted blob versions.

#### Asking for a new link

**Shipped 2026-08-06.** `POST /api/claim/resend`, anonymous, offered on the claim page's `expired` and `superseded` panels.

**A claim token and the letters it points at expire on different clocks, and that gap is the whole reason this exists.** The pending site's window rolls forward with every letter that arrives; the token minted alongside letter one does not. So the ordinary case is somebody written to in March who goes looking for the email in June and holds a dead link to letters that are very much still there. "Wait for the next letter" is a true answer and a week-long one.

- **The dead token is the credential.** We signed it, so holding one proves the holder was sent a claim email; verifying it *without* the expiry check is the entire mechanism, and it needs no new state anywhere.
- **Nothing is disclosed either way.** The new link goes to an address already in the manifest — the last one offered, which is by definition where the link being invalidated went — and the response never names it. This can ask that somebody be written to; it cannot be used to find out who.
- **Rate-limited by a quiet hour rather than a cap on the count.** The same counter records the ordinary offers, so a site legitimately offered to its three forwarders would arrive at this page already forbidden — refusing the one person actually asking, on the strength of mail they never received. A window refuses nobody permanently.
- **A live link may be resent too.** Somebody who has mislaid the email cannot tell a stale link from a live one and should not have to. The only cost is that the mislaid link stops working.
- **Refused once the site's own window has run out**, because a fresh link would be born expired. The [grace period](#the-retention-window-is-rolling-and-it-upgrades) means the letters are still on disk at that point, so the page says so plainly rather than promising a link it cannot mint.
- **Pending sites only.** A `claim@` or relay grant already has `claim@pdayletters.com` as its resend path, and that one is better: it proves control of a mailbox rather than possession of a link.

#### Re-inviting the missionary

**Shipped 2026-08-06**, in `offer.js` as `invitationDue`, called from ingest.
...
**Tracked as `claimEmailSentAt` and `claimEmailCount` on `claim.json`** — not, as originally planned, on the missionary's `users` row. The row would have let the schedule survive a pending-site purge and recreation; the manifest needs no new state, no new write and no second source of truth, and by the time a purge has run it has deleted the letters the invitation was about. Starting the schedule over at that point is the honest behavior rather than a shortcoming. On claim both stop mattering; on an active site the missionary hears nothing at all.

**The forwarder's chase is separate and lives elsewhere.** `hasDirect` is the switch: a site the missionary writes to directly is chased by ingest, in reply to letters he is still sending, on the schedule above. A forward-only site has nobody to reply *to*, so it gets [the one reminder](#phase-7--onboarding-pending-sites-and-the-claim-flow) from the nightly `remind` timer instead. A site that becomes direct after a forward hands over from one to the other, which is the right moment for it: there is now somebody to write back to. Nothing is chased twice, and there is no "already reminded" flag to forget to write — sending pushes `claimEmailCount` past one, which is the reminder's own condition.
...
**But only if we reply from the address they actually wrote to.** The prior-correspondence signal keys on the sender address, so a reply arriving from an unfamiliar address throws away the very thing that makes it solicited. Replies to inbound mail must therefore:

- Set **`From:` to the address the sender wrote to** (`claim@pdayletters.com` or `post@pdayletters.com`).
...
- Keep the **envelope sender (`Return-Path`) on `cf-bounce.pdayletters.com`** for bounce handling. DMARC still passes via DKIM alignment (`d=pdayletters.com` matching the `From:` domain).

Bounces and stray replies then arrive at `claim@`/`post@` and flow through inbound parse, where the classifier drops them like any other unroutable mail. Purely-generated mail with no inbound message to answer — reminders, ownership nudges, invitations — sends from `hello@pdayletters.com`, since there is no correspondence signal to preserve.
...
- **Nothing renders before a claim.** An unclaimed site has no rendered artifacts, no ACL, and no URL that resolves — so a spam-triggered pending site is inert, cheap, and self-cleaning, and expiry deletes one prefix rather than reconciling four.
- **Dedup is deferred to claim time**, because its source of truth is `posts.json`, which does not exist during pending.
- **A signed claim token is genuinely necessary here**, unlike an unsubscribe link whose recipient is already an ACL member. At claim time there is no ACL to check against, so the token *is* the authorization. The same mechanism, scoped to one slug and one role, backs [invitations](#invitations).
...
- **Both fields have a length, and for a long time only one of them did.** `MAX_SUBJECT` is 500 and has always carried a comment saying it is short enough that the field cannot be used to push a megabyte into `posts.json` — which was true of the subject and quietly false of the body sitting next to it. `MAX_BODY` is now 128 KB. The number is measured rather than guessed: across one complete real archive of forty-four letters the longest body ran to 5,138 characters and the median to 1,479, so the cap is about twenty-five times the longest letter anybody has actually written. Checked before sanitizing, since the sanitizer's cost is proportional to what it is handed.
  - **What it is really protecting is everybody else's page load.** `posts.json` is served whole, on every visit, to every reader, and is baked into every offline export and typeset into every book. An unbounded field there is not one owner's problem.
...
**Shipped 2026-08-06.** `POST /api/posts/{slug}/{postId}/restore`.
...
- **Every edit on that post is discarded, including edits made by a different owner.** The confirmation says so plainly and names them: *"This discards every change made to it, including sarah@example.com's, and any pictures added to it here."* There is no per-field restore and no diff to review — both would need a revision history, which deliberately doesn't exist.
- **Pictures an owner added here go with the edits** — see [Owners can add pictures to a post](#owners-can-add-pictures-to-a-post). The alternative, asking whether to keep them, was rejected: it needs a second code path, a second stored state distinguishing an added photo from a mailed one, and a prompt on top of the confirmation that is already there. Discarding them needs none of that, because the restore rebuilds `photos` from the message that arrived and that message never had them. *Restore means restore* is the section's whole principle and this is the same rule applied to one more field. An **ordinary re-render is the opposite** and deliberately so: nobody asked for that one, so it carries them across.
...
- **Offered only on a post somebody has edited.** On an untouched letter the restore re-renders it into exactly what it already says, so the button would be a permanent fourth control for a no-op. `editedAt` is the test.
- **Run in the request rather than queued,** unlike every other render. The owner has just confirmed a destructive action and "it is happening somewhere, probably" is the wrong answer to give them; the work is bounded by one message's photos, which are content-addressed and rewrite the same blobs.
- **The same pass does both jobs, with a flag.** `runRender` takes `restore`, and only under it does it write the subject and clear the stamps. That distinction is the reason the flag exists rather than always restoring: an operator re-render after a sanitizer fix must not undo a subject an owner deliberately rewrote, nor put an owner's name on a change they did not make.
...
- **Owners get them, marked.** The same endpoint returns hidden posts to `owner` callers with the flag intact, and the reader draws them dimmed with an **Unhide** action — so a post can be taken out of view without being lost track of. Dimmed rather than badged: the badge was a pill on its own line above the heading, which turned an archive with several held letters into a column of warnings. The reason moves into the summary line beside the photo count, because dimming can say a letter is out of view and cannot say why, and why is the whole of what the owner needs. An open letter is un-dimmed, since the owner is reading it in order to decide about it.
...
- **24 to 500 pages, even.** An odd count silently prints a white back cover. Short missions are padded up with blank leaves; long ones are refused, and the two are not symmetrical. A thin book cannot be told how much more it needs, because that depends on letters nobody has written yet, so it is padded. A fat one knows exactly how far over it is — and the album prints six photographs to a page, so the arithmetic back to a number of pictures is the whole of the message. The check runs between the builder's two passes, where the layout is known and no photograph has been fetched yet, so an unprintable book costs arithmetic rather than several hundred megabytes of downloads. `SHEET_MOST` is a parameter of `buildInterior` for the same reason `SHEET_LEAST` is: the rule belongs to whoever is binding the book. **Splitting a long archive into two volumes is the other answer and is not built** — it is two PDFs, two publications, two prices, two covers, a split point that every new letter can move, and a one-to-many `books` layout. Telling an owner what to remove is a sentence; the split is a feature. One complete real archive lands near 200 pages, so the ceiling is not close.
...
- [x] ~~One live end-to-end run from a real `@missionary.org` account~~ — [Phase 6](#phase-6--direct-ingest)
...
- [x] ~~Recurring reminder and tapering invitation series~~ — [Phase 7](#phase-7--onboarding-pending-sites-and-the-claim-flow)
- [x] ~~"Email me a new link" for an expired token~~ — [Phase 7](#phase-7--onboarding-pending-sites-and-the-claim-flow)
...
- [x] ~~Restore a post to its original~~ — [Phase 9](#phase-9--owner-admin-invitations-and-operators)
...
- [x] ~~`/manage/last-received` service-wide flow view~~ — [Phase 9](#phase-9--owner-admin-invitations-and-operators)
- [x] ~~Owners can add pictures to a post~~ — [Later features](#later-features)
...
- **`main.bicep` first deployed on 2026-08-05**, having described the infrastructure for months without ever being applied. The static site and the Worker shipped themselves on a push; the template had no such path, so it accumulated changes made directly against the live account.

  **Two things about `what-if` came out of that first deployment:**

  - **It does not diff `siteConfig.appSettings` at all.** A `GET` on `Microsoft.Web/sites` does not return them, so `what-if` has nothing to compare and reports nothing — not a warning, not an unknown, silence. `main.bicepparam` had never set `cloudflareAccountId` or `mailAllowlist`, both of which default to empty in the template, so a deployment would have blanked `CLOUDFLARE_ACCOUNT_ID` and `MAIL_ALLOWLIST` and reported success. Because an empty allowlist means *mail nobody* by design, outbound mail would have stopped without erroring. Both are now pinned in the parameter file. **App settings have to be diffed by hand.**
  - **Its `Modify` count is not a drift measure.** It reported 28 `Modify` before the deployment and 28 immediately after a successful one, against a resource group that matched the template exactly. The residue is server-populated defaults the template omits (`defaultEncryptionScope`), read-only properties (`stableInboundIP`), and `reference()` expressions `what-if` cannot resolve. Only the *list* is usable, inspected property by property.

  The deployment did fix one real gap: the template omitted `repositoryUrl`, `branch`, and `provider` on the Static Web App, so it proposed unlinking the GitHub repository. Those are now declared.

  **`deploy-infra.yml` applies the template on every push touching `infra/**`.** It authenticates by workload identity federation, so **no Azure credential is stored in GitHub**. The identity is `mj-id-deploy`, a user-assigned managed identity rather than an app registration, chosen because an app registration can have a client secret added to it later and a managed identity cannot.

  **The federated subject is not what the documentation says it is.** Nearly every example writes `repo:owner/name:ref:refs/heads/main`. GitHub's current default embeds numeric owner and repository IDs — `repo:kurtzeborn@22382549/mission-journal@1311226429:ref:refs/heads/main` — so a credential written the documented way fails with `AADSTS700213` and an error naming a subject that appears in no configuration file. Read it rather than assemble it: `gh api repos/{owner}/{repo}/actions/oidc/customization/sub` returns `sub_claim_prefix` directly. The IDs survive a rename and cannot be inherited by whoever registers the name after a repository is deleted.

  **What that identity may do is the real question.** The template creates role assignments, which `Contributor` cannot do, so the deploy identity also holds `Role Based Access Control Administrator` at resource-group scope. Unconstrained, a push to `main` could grant any role to anything — including handing back the `permanentDelete` capability the split above exists to remove. Two things narrow it:

  - **An ABAC condition restricts it to the six roles this template assigns.** It cannot grant `Owner`, `Contributor`, or `User Access Administrator`. The limit worth knowing: a condition can name roles but not scopes, so re-granting `Storage Blob Data Owner` account-wide is still expressible.
  - **Its own two grants are deliberately not in the template.** Declaring them would make every run re-assert them, forcing the condition to permit granting `Contributor` — and a workflow that can grant `Contributor` to anything is not constrained at all. The credential that runs the deployment is therefore not grantable by that deployment.

  **The workflow checks the service, not just the deployment.** ARM reporting `Succeeded` means the template was accepted, which is weaker than it sounds: a blanked parameter deploys perfectly and breaks the app. The run finishes by calling `/api/optout/describe`, `/api/claim/describe`, and `/api/invite/describe` and failing on anything but `200` — three routes that between them require the Function App to have started, read its settings, and resolved a Key Vault reference.

- **`deploy-functions.yml` ended hand-publishing the API.** Infrastructure, the static site and the Worker all deployed themselves while the Function App — the component that reads other people's mail — was published from a laptop. What ran in production was whatever happened to be in one working tree, which no commit records; and the test suite gated nothing, so a red run and a green run shipped alike.

  **Test and deploy are jobs in one workflow rather than two joined by `workflow_run`.** `workflow_run` fires when the first workflow *finishes*, not when it passes, and it resolves the code to deploy independently of the code that was tested. `needs: test` has neither gap.

  The deploy job runs no `npm ci`. Flex Consumption builds remotely, so the zip carries `package.json` and `package-lock.json` and Oryx installs from the lock on the platform side. It authenticates with the same federated identity as the infrastructure workflow and finishes by asking the running host to answer rather than trusting the deployment API.

  **With no publish profile in use, basic publishing credentials had no consumer, and both were still open.** The `scm` and `ftp` endpoints each accept a username and password Azure mints for the app. That pair belongs to no person, never expires, and is not revoked by anything. **Code deployed there runs as the app**, and the app's identity reads the storage account and resolves Key Vault references — so deploy rights are read access to every letter, reached without touching Entra, RBAC or MFA. Both are now declared `allow: false` in `main.bicep` rather than switched off in the portal, so the state is re-asserted on every infrastructure run.

- **No dependency can change without a commit, and that was measured rather than assumed.** Both `package.json` files carry committed lock files, and every workflow that installs runs `npm ci`. The uncertain path was the Function App, which builds remotely with Oryx — nobody writes `npm ci` there, so whether `package-lock.json` is honored is a property of someone else's build system.
...
- **The browser libraries were outside all of this until 2026-08-21, and they are the ones a family actually runs.** MiniSearch, wordcloud2 and Font Awesome sat under `web/vendor` as files somebody had downloaded by hand. Nothing recorded which version they were beyond a comment, nothing would have said so if one were superseded, and Dependabot could not see them: it reads `package.json` files, and `web/` had none. **The only dependencies in the system that execute on a reader's machine were the only ones with no supply-chain signal at all.**
  ...
  **This was cheap because it was checked rather than assumed.** The three packages were installed at the versions the hand-downloaded files claimed, re-copied by the script, and diffed: every library body and every license came out byte-identical, so the hand copies had been faithful and nothing shipped to a reader changed. Only the headers moved.
...
- **The storage identities are split by scope so `permanentDelete` never reaches `raw/` from the credential exposed to inbound mail.** Checked against the live account on 2026-08-05, the Function App's identity held **Storage Blob Data Owner**, account-wide, and had done since this phase — so the identity processing attacker-supplied mail could permanently erase the one part of the system that cannot be rebuilt.
  ...
  **The grant was deliberate, which is why it survived.** `main.bicep` carried a comment on it: *Blob Data Owner rather than Contributor: the Functions host manages its own leases and the deployment package container, which Contributor cannot do.* Microsoft's guidance says the opposite — that `Storage Blob Data Contributor` scoped to the deployment storage account is what a system-assigned identity needs. The documentation cannot settle which is right, so the split avoids having to answer it:
  ...
  - **Storage Blob Data Owner, on `azure-webjobs-hosts` and `azure-webjobs-secrets` only** — the containers the Functions host runs for itself on the archive account. Neither holds a letter. The deployment package moved to its own account on 2026-08-30 and carries the same grant scoped to its `app-package` container there.
...
**Shipped.** 25 real letters ingested; 14 fixtures in the corpus.
...
- **That holds for forwards, and only for forwards.** A forward is re-composed by the forwarder's client, so a corpus of forwards characterizes forwarders and says nothing about how a missionary's own phone writes a letter. Gmail's Android composer marks up paragraphs as bare `<div dir="auto">` blocks with no margins and puts the blank line between them in a `<div dir="auto"><br></div>` of its own — a shape no forward in the corpus contained, and one the sanitizer deleted as spacer noise, publishing a real letter as a single paragraph. `direct-gmail-android.eml` is that capture. The general rule: **one real send from each client anyone actually composes on**, because the compose path and the forward path are different code inside the same app.
...
- **A real letter is scrubbed by replacing the whole body, not by replacing names in it.** When a genuine message is worth keeping as a specimen, map each decoded part body in its entirety to the placeholder letter, written out with the client's own markup shape. Replacing only the identities inside it leaves the correspondence in a public repo, and paragraph-by-paragraph maps cannot work anyway: the `text/plain` part is hard-wrapped where the `text/html` part is not, and the same sentence appears in both with different apostrophes. **Body entries must come first in the map.** They are matched whole and the sign-off inside them contains the display name, so a name entry running first rewrites the body out from under its own key — the wholesale swap then misses silently and the scrub reports clean.
...
- **The DKIM measurements are written down** in [docs/dkim-findings.md](dkim-findings.md) rather than living in commit messages. Every table there is asserted by `functions/tests/dkim.test.js` and `functions/tests/arc.test.js`, so a client changing its behavior surfaces as a test failure instead of a silent loss of coverage.
...
- **The Entra registration needs `enableIdTokenIssuance`, and Bicep cannot say so.** SWA custom auth asks for `response_type=id_token`; `az ad app create` leaves that grant off, so Entra answers 700054 and returns the visitor to `/.auth/login/aad/callback` with no session. Set it with `az ad app update --id <appId> --enable-id-token-issuance true` and treat it as part of *creating* the registration — a rebuild from `main.bicep` will not restore it, because the property lives in Graph rather than ARM. Google worked throughout, which pointed the diagnosis at the Microsoft *configuration* rather than the *registration*.
...
**The zip is a lesser experience on purpose, and that is settled.** It is a *preservation* format: the letters, the photos, and something that opens them, on a family's own disk, still working when none of this is running. It is not a second copy of the website. The live site is where the reading actually happens, and it gives up nothing to keep the zip in step — where a feature cannot follow offline, the feature ships and the zip does without.

The one thing that does not degrade is the letters themselves: every letter, every photo, readable and searchable, because that is the whole promise. What has to stay true beyond that is narrow — `web/reader.js` runs unmodified in both places, so anything on the path that draws a letter has to *load* from `file://`, which means no ES modules and no `fetch` there. Everything else is a preference.

**It has been mistaken for a constraint more than once**, and the cost is real: it was used to rule out a bundler for the letter editor, which never ships in the zip at all, and to argue against a packed word cloud that turned out to be 37 KB and more accessible than the list proposed instead. Both are corrected below. Before citing `file://` against a design, check whether the thing being designed is even in the archive.
...
- **That constraint is now designed out: the Function stages the zip in blob storage and hands back a link.** `download` builds the archive straight into `exports/{slug}/{role}.zip` and answers `302` with a 15-minute, read-only, single-blob **user-delegation SAS**. The Function is then on the hook only for the build, which is fast and in-region; the transfer moves to storage, which serves `Accept-Ranges: bytes` and so can be *resumed*. Measured on the real 24-letter site: 20 MB, 105 entries, built and uploaded in **4.2 s**, fetched back in **0.8 s** against ~7.6 s streaming it through the Function. The earlier 400–500 MB estimate assumed ~10 photos per letter; the real density is ~2, so a full 104-letter mission projects to roughly 80 MB — well inside the response window even before the transfer was moved off it.
  ...
  - **Loose end, closed 2026-08-06:** `exports/` accumulates. Each rebuild overwrites, but versioning is on and every overwrite retains a 30-day version. `expire-exports` deletes blobs, snapshots and versions after **7 days** — an export is a second copy of an entire family's correspondence sitting under a URL somebody was emailed, and seven days is the shortest window that still covers a link nobody opened until the weekend.
...
**Shipped.** The classifier's `direct` branch is implemented and returns `publish`; the corpus carries three direct captures — `direct-missionary`, `direct-bcc-inline-via-cloudflare`, `direct-bcc-inline-via-exchange`. The `originalDate` ordering is fixed and covered, warning-level logging of a missionary-domain rejection is built, and an unclaimed direct send is held as a pending site.

**The live run happened on its own, which is how it was always going to happen.** On 2026-08-14 a real missionary BCC'd `post@pdayletters.com` from `@missionary.org` on his weekly letter — `To: undisclosed-recipients:;`, so the BCC path and the direct path in one message — and it published unattended to the slug a forward of his previous letter had already created. Cloudflare's own verdict on it: `dkim=pass header.d=missionary.org header.s=google`, `dmarc=pass header.from=missionary.org policy.dmarc=quarantine`, `spf=pass`. That is every line of the verification below, including the one that mattered most: **a direct send and a forward of the same person's mail reach the same slug.** Nothing was gated after all; the gate was waiting for somebody to write home.
...
- **Do not spend the window on the OAuth question.** Whether an `@missionary.org` Google account can sign in to a third-party app looked like a gate and is not one: [ownership binds a personal account by design](#the-60-day-cliff), so a `no` costs nothing and a `yes` is a case already handled defensively. Missionaries reach that mailbox by signing in to the Church's own site and following a link through to Gmail, so they never hold a Google password to give a consent screen.
- **`direct-missionary.eml` in the private corpus already answers the gating question.** It is a genuine `@missionary.org` send that reached Cloudflare, and its `mx.cloudflare.net` verdict is `dmarc=pass header.from=missionary.org policy.dmarc=quarantine`, with `dkim=pass header.d=missionary.org header.s=google`. **Check the corpus before spending a scarce credential.**
...
- ~~**Run the deferred Phase 0 checklist**~~, which this phase exists to unblock: ~~confirm a genuine DMARC pass on Proofpoint-relayed missionary mail~~ **(confirmed live — `dmarc=pass` against a `p=quarantine` domain)**; confirm a threaded reply reaches the inbox rather than spam; ~~test whether an `@missionary.org` Google account can sign in to a third-party OAuth app~~ **(not a gate — ownership binds a personal account by design)**.
...
- **`originalDate` ordering is a byte comparison, not an instant comparison.** `presentPosts` originally compared `originalDate` as a whole string, which was correct only by accident. The obvious fix — compare parsed instants — is worse: a missionary transferred from `+08:00` to `-07:00` writes a letter headed August 2 whose instant *precedes* one headed August 1, so instant ordering lists August 2 above August 1 while the page prints those dates beside them. An inline forward carries no offset at all, so `Date.parse` invents the *host's* — making the order of a family's letters depend on where the code runs. The comparison is an explicit byte comparison of the first 19 characters, ties broken on `id` so the order is total and identical between requests, with four tests over the transfer case, the mixed offset-free case, the tie, and an undated post. The reader agrees, splitting the date rather than parsing it. **"Compare instants" is wrong wherever the local calendar day is the thing being displayed.**
- ~~**Verification:** send a `direct` message and confirm it publishes to the same slug that a forward of the same letter reaches.~~ **Done, unprompted, on 2026-08-14.** See the note at the head of this phase.
  - **The live message found a bug the fixtures could not.** Gmail's mobile composer writes each paragraph as a bare `<div dir="auto">` and puts the blank line between them in a `<div dir="auto"><br></div>`; the sanitizer's empty-block pass removed those as spacing, and the letter published as one unbroken wall of text. Every fixture in the corpus was composed on a desktop client that spaces paragraphs with margins instead, so nothing in 862 tests could have caught it. **Fixed:** a block holding a `<br>` and nothing else is a paragraph break and is kept, unless it comes before the letter's first text or after its last, where it separates nothing. **The lesson is about the corpus, not the sanitizer** — every capture in it was a forward, and a forward is always re-composed by the forwarder's client. The one thing a real send tests is the sender's client. **The message is now `direct-gmail-android.eml`**, scrubbed down to the placeholder letter but keeping every byte of that markup shape, so the corpus covers the composer as well as the forwarders.
  - ~~**The `missionary.org` DKIM public key is still resolvable** at selector `google`.~~ Re-resolved 2026-08-16 and byte-identical to the 2026-08-04 capture already in the private repo, so nothing new needed writing down. The unscrubbed message sits beside it as a second pristine specimen — a direct send this time rather than a forward — and the reason to keep it is that this plan leans on re-verifying signatures on letters forwarded years later, which becomes permanently untestable the moment the key rolls with no copy kept.

### Phase 7 — Onboarding: pending sites and the claim flow
**Shipped.** Accumulation landed early in Phase 6; the claim half landed after it. Tokens are HMAC-signed with a key from Key Vault and only their hashes are stored; `/claim` describes a pending site to an anonymous visitor and redeems it for a signed-in one; redemption spends the token before it grants anything, creates `acl.json` with `If-None-Match: *`, writes a membership row, and promotes the whole backlog through the same code path that commits a live letter.

**The last two pieces landed 2026-08-06:** the tapering invitation series for a missionary whose own letters created the site, and "email me a new link" for a stale one. With them came a `remind` timer for the forward-only case, at 03:45 UTC, half an hour after the purge so a site being deleted tonight is gone rather than chased on its way out. The claim email itself, the `claim@` handler and the "a site already exists" reply all shipped with [Phase 8](#phase-8--outbound-mail-and-preferences).

**The expiry purge has shipped** as a nightly timer. It is the only code in the system that destroys a letter, so every ambiguity resolves to *keep*: an unreadable manifest, an unparseable expiry, or a claimed site that still holds letters are kept and logged rather than swept. It deletes the letters **before** the manifest, so a crash partway leaves a still-expired manifest the next run finishes off, rather than `.eml` files nothing will ever list again. **It waits seven days past `expiresAt` before acting**, because `describeClaim` returns the slug for an *expired* token so the page can offer a fresh link, and purging at the instant of expiry would make that offer unkeepable. A site that expires having never been emailed is logged at error level and separately from the routine count: that is our bug rather than a family's silence.
...
- ~~**De-duplication**, which promotion cannot work without.~~ **Already built.** `findDuplicate` takes a candidate and a list, so promotion gets batch behavior for free by committing letters one at a time against an accumulating `posts.json` — the same loop, the same ETag, the same gate. The batch dedup this bullet called for did not need to be written.
...
- **Promotion must never re-classify.** It reuses the *commit* half of ingest and takes the verdict as an argument rather than recomputing one. Re-classifying would mean re-verifying DKIM against whatever key the sending domain publishes *now*, possibly months later — and domains rotate keys. A letter that verified on arrival can fail verification later through nobody's fault, and the cost of that would be discarding the letter the whole mechanism exists to preserve. Splitting `commitLetter` out of `runIngest` was what made this possible; it is behavior-preserving and the existing suite proved it.
- **Pending sites and claim** per [Onboarding and auto-provisioning](#onboarding-and-auto-provisioning). Shipped: the anonymous landing page showing sender, waiting count and sample subjects before sign-in; the first owner; the display name; promotion of accumulated raw; the failure page; the rolling `expiresAt` and the timer-triggered purge. **The sample subjects and the count are recorded on `claim.json` as letters arrive**, rather than computed by opening every held message on a page load, which would time out for exactly the families with the most to lose. Capped at three, because a stolen link should not hand over a summary of the whole archive. **Complete as of 2026-08-06**, with the invitation series and the resend offer.

  **The address the claim link is sent to must come from the envelope, never the body.** `holdPending` was recording `extracted.original.from` as the pending site's sender. On a *forward* that is right and is the point of the extractor. On a *direct* send there is no forward to see past, and the extractor's fallback still fires: a body line beginning `From:` is what a mail client leaves behind when it flattens a forward, so a missionary quoting a message from home had the quoted address recorded as his own. `classify` was never fooled — the direct branch keys off the authenticated envelope sender as `verdict.author` — but the pending manifest took the extraction, and the claim link goes wherever the manifest says. **That is a credential routed by attacker-supplied text**, in a design whose central claim is that no such value exists. The exploit is weak (you must authenticate as `@missionary.org` first, so you are attacking your own site); the mistake is not (a quoted email sends your claim link to your mother). Fixed to use `verdict.author`, with a regression test confirmed to fail before the change.

  **It was invisible for as long as it was harmless.** The field was recorded and never read from Phase 7 until Phase 8 turned it into a delivery decision, and the [allowlist](#phase-8--outbound-mail-and-preferences) then hid it again by refusing to send anywhere unlisted. Both defenses would have stopped working the day the allowlist opened. **A value nothing consumes is verified by nothing, so the commit that first consumes it is where its history gets audited.**
...
**The send layer, the claim email, rejection notices and opt-out are shipped; per-user preferences are not started.** The provider is **Cloudflare Email Service**, which has a `send_email` Workers binding, a REST API, SMTP, a suppression list, and Google Postmaster feedback. Every header this phase needs is allowlisted.

**Sending goes through the REST API from the Function, not the Workers binding.** Phase 7 asked for the claim email to be sent "as a reply to an arriving letter", which reads as a requirement for `message.reply()`. It is not: threading is a property of `In-Reply-To` and `References`, which are ours to write, so a REST send reproduces it exactly. `reply()` adds only failure modes — the incoming message must pass DMARC, one reply per event, the recipient must equal the incoming sender, the domains must match, `References` must be under 100 entries — all of which land when somebody's first letter arrives. It would also contradict the Worker's principle that anything deferrable to ingest is deferred, since a failure there retries from durable storage.

- **Nothing sends unless `MAIL_ALLOWLIST` names the recipient, and empty means nobody.** This defaults closed while the [purge timer](#phase-7--onboarding-pending-sites-and-the-claim-flow) defaults open: there, forgetting a flag can only cause deletion, and the dangerous outcome is silence; here, every recipient is computed from headers a stranger wrote, and the dangerous outcome is mailing that stranger. `*` opens it and has to be typed on purpose.
  - **It is `*` in production.** Two named test addresses sufficed while the only outbound message went to a mailbox we controlled; `claim@` answers missionaries we have never corresponded with, and would have failed as a *blocked* send nobody was watching for. The code still defaults to nobody, so a fresh environment is silent until somebody widens it.
- **Nothing in the send path logs a body, a subject, or a full address.** The claim email's security model is that its token is seen only by its recipient, and the token is in the body. Recipients are masked to `s***@domain`. The `fetch` failure path drops the error message, because a fetch error can quote the request, which carries the API token in a header and the claim token in the body.
- **Minting a claim token and recording that one was sent are two separate writes.** They used to be one, so `claimEmailCount` incremented whenever minting succeeded, sent or not — and the purge timer reads that field to decide whether it is deleting letters from somebody who was never told. A broken send would have produced a manifest asserting the offer had been made.
- **A pending site is offered once, on the letter that created it.** Minting invalidates the previous token, so re-offering on every letter would hand a weekly writer a fresh credential and break the link they finally got round to clicking. The condition is `claimEmailCount === 0` rather than a flag, which makes a failed send self-healing.
- **The claim email threads against the newest held letter, not the first.** `lastMessageId` is kept on the manifest for this. Absent when the sending client wrote no `Message-ID`, which threads nothing and breaks nothing.
- **A permanent bounce is a failure, not a success.** The API call succeeds and delivers nothing.

Prerequisites, all met:

- **`pdayletters.com` is onboarded for *sending* as of 2026-08-04.** `cf-bounce` carries the bounce MX, an SPF record and a DKIM key on its own selector, separate from the routing records on the root. **The onboarding screen proposed replacing `_dmarc` with a bare `v=DMARC1; p=reject;` and then did not do it** — the existing `p=none` record and its `rua=` survived, despite a confirmation screen that reads as though it will remove conflicting records.
- **Workers Paid is required.** Sending to arbitrary recipients is unavailable on the free plan. 3,000 outbound emails are included per month, overage is $0.35 per thousand, and sends to verified destinations never count against the quota.
- **Email preview was turned off on 2026-08-04**, before any send path existed. It retains full message content for about seven days in the activity log. Re-check after any Email Service settings change: it is the one setting that silently converts a credential into a logged one.

Shipped since, and previously listed here as outstanding:

- **Rejection notices.** `rejection.js`, called from `runIngest`. An ACL member whose message is refused is told why; silence is correct for strangers and wrong for someone who can already read the site, since a reader who forwarded inline otherwise concludes it worked and loses the letter. Rate-limited to one message per sender per reason per day through the `nudges` table.
- **The "a site already exists" reply**, in `offer.js`, and the relay path in `relay.js`.
- **One-click opt-out** without sign-in, `POST`-confirmed per the scanner problem in Phase 7. `optout.js`, with the `optouts` table storing address hashes.
- **Ingest nudges**, in `nudge.js` — the reply sent when a bootstrap forward fails DMARC, offering the relay link second. A single reply, distinct from the invitation series in `offer.js`.
- **The `remind` timer**, in `offer.js` as `remindPending` — the one chase a forward-only pending site gets, seven days after the offer that was never taken up.
- **The forward acknowledgement**, in `ack.js`, called from `runIngest` after `commitLetter`. See below.

**The acknowledgement is one message, not two.** The plan called for a post-published ack and a separate dedupe ack, and they were collapsed deliberately: from the forwarder's side both are the same news. They sent a letter to be archived and the letter is in the archive. That somebody else got there first is a fact about our storage, and explaining it invites the reply "so was it *my* copy?" — a question with no useful answer.

- **It answers a message and never fires on a schedule.** Only forwards; a direct send is the missionary's own letter arriving at their own archive, and answering it would be telling them what they just did.
- **One per person per archive per UTC day, however many letters they send.** The shaping scenario is a family forwarding two years of letters in one afternoon: a hundred receipts is what a mail provider reasonably concludes is an attack, and it is a worse answer than none to the question the receipt exists for. The row in `nudges` is both the record and the lock, matching `nudgeOnce` and the daily cap's UTC boundary.
- **The gate is not the preference.** `postAckEmails` / `dedupeAckEmails` on a `users` row are still unbuilt, because the settings page that would set them is unbuilt and a preference nobody can change is not a preference. The daily gate and the existing opt-out stand in until then — and the opt-out is honored here, since a forwarder who asked us to stop is exactly the person a well-meant receipt would annoy most.
- **It is hooked in `runIngest`, not in `commitLetter`.** Promotion calls `commitLetter` directly, and a claimed site replaying a year of held letters must not thank a dozen forwarders months late for something they had long forgotten.
- **The received `Message-ID` is stored on the post as `receivedMessageId`**, alongside `originalMessageId`. They are not interchangeable: the original identifies the *letter* and is what dedupe compares; the received one identifies the *conversation* and is what `In-Reply-To` must name. Reading the header at the call site rather than off the post is deliberate — the duplicate path returns no post.
- **The link is offered without a promise that it will work.** A forwarder is frequently not a member. Sending them to a sign-in page that then refuses them would turn a receipt into a rejection.
...
- ~~**Suppression has to be visible to owners, not just to us.**~~ **Shipped 2026-08-06**, as `delivery.js` and the `deliveries` table, keyed by SHA-256 of the address exactly as `optouts` is.
  - **There is no suppression status, deliberately.** Cloudflare's REST error table runs 10000–10203 and none of the codes is suppression; `E_RECIPIENT_SUPPRESSED` is a *Workers-binding* string error and this service uses the REST API. A `suppressed` status matching `/suppress/i` was built and then removed: the match was a guess at wording we have never seen, and all it bought was a differently-worded sentence on a page that was already going to say this address is not receiving mail. Every rejection is `failed`, and the provider's own words survive in the log. **The owner's remedy is the same in every case** — check the address, then ask the person.
  ...
  - **It is advisory and nothing reads it to make a decision.** A second copy of Cloudflare's judgment could only ever be wrong in the direction of silently declining to write to somebody whose address started working again.
  - **The row is keyed by address, not by site**, because the suppression is. The address is stored in the row as well as hashed into the key: a table of hashes cannot answer "why did grandmother stop hearing from us", which is the entire question.
  - **On the page it dims rather than badges**, the same 0.55 as a hidden letter, with one sentence on the line in ordinary words. **The controls stay at full contrast** — Resend is often the next thing to press, and dimming the remedy along with the problem would be the wrong half. The color is warm rather than red, because an alarming color beside a grandmother's address reads as an accusation.
- **`Message-ID` cannot be set on our own mail** — it is platform-controlled and generated on a Cloudflare domain. Threading still works, because `In-Reply-To` and `References` are ours to write, but nothing downstream may assume it can recognize its own outbound `Message-ID` later. This is also why an *inbound* `Message-ID` is worth storing: it is the only handle any of this ever gets.
- **Outbound sends appear as "dropped" in the Email Routing summary even when they were delivered.** Read [Email sending metrics](https://developers.cloudflare.com/email-service/observability/) instead.
- **The Email Sending settings page reports a DMARC policy the domain does not have.** Its DNS list shows `_dmarc` as `v=DMARC1; p=reject;` while the published record is `v=DMARC1; p=none; rua=...`, confirmed against two independent resolvers. The `Unlocked` status beside it is the true part — Email Service is not managing that record — but the value column for an unlocked row shows what Cloudflare *would* write, not what exists. **Check DMARC policy in DNS, never on that page.**
- Per-user settings page at **`/settings`**, not `/{slug}/settings` — these are columns on one `users` row, and a per-site URL implies a per-site scope that doesn't exist. Distinct from the per-archive `/settings/{slug}` page that already exists.
- ~~Per-slug daily ingest cap with alerting, so a mail loop or a forwarding rule gone wrong can't quietly generate thousands of posts and a matching storage bill.~~ **Shipped 2026-08-06 at 200 letters per archive per UTC day.**
  - **The number is set by the honest extreme, not the abusive one.** A two-year mission is about a hundred weekly letters, and forwarding the whole lot in one sitting is what a family does on the day they find out this exists. 200 clears that and still stops a loop two orders of magnitude short of "thousands".
  - **It counts rows in an `arrivals` table, one per letter, rather than incrementing a number.** Read-add-write loses increments whenever two messages are in flight, and the queue host runs a batch at a time — so under exactly the flood this exists to catch, the counter would advance more slowly than the mail arrived. A row per message cannot undercount. It can overshoot by as many messages as are in flight, which is the harmless direction.
  - **The check sits in `runIngest`, above both the pending branch and the commit.** A loop into an unclaimed site is worse, because there is no owner to notice. **Promotion is exempt for free**, because it calls `commitLetter` directly — a claimed site replaying a year of held letters must never be capped, and those letters were already counted on the days they arrived.
  - **A refused letter is not a destroyed one.** The raw message stays in `inbox/` under its 30-day lifecycle rule, so a cap that fires wrongly is undone by re-enqueuing the ULID.
  - **It fails open.** A table refusing reads is not evidence of a loop, and this is the one guard whose malfunction would otherwise reject real mail.
- **An archive holds 150 letters, ever.** The daily cap limits the rate and not the total, and those are different guarantees: 200 a day sustained is seventy thousand letters a year, in one JSON document that every reader downloads on every visit. Nothing pages it, deliberately — the client-side search rests on having the whole archive in hand — so "an archive is small" was a load-bearing assumption written down nowhere and enforced nowhere. `ARCHIVE_CAP` is now the place it is written down.
  - **The number comes from the calendar.** A two-year mission is about 104 p-days, so 104 weekly letters is the theoretical ceiling for an elder and rather fewer for a sister serving eighteen months. The one complete archive on the service ran to forty-four. 150 clears the theoretical maximum with room for the things that are not weekly letters — a mission president's note, a transfer announcement, a re-forward an edit stopped dedupe from matching.
  - **The check is in `commitPost`, after the duplicate check and before the append.** That order matters: a family who forwards the same letter twice into a full archive should be told it is already there, not that the archive is full, because the second answer sends them looking for a problem their own mail did not cause. It also means the check reads a `posts.json` it was already holding, and costs nothing.
  - **Unlike the daily cap, this one does not reset tomorrow.** Reaching it is permanent until somebody deletes a letter. The replay path is the same — the raw message keeps its thirty days in `inbox/` and can be re-enqueued by ULID — but that window is the only warning there is, which is why the refusal logs at error level.
  - **The owners are emailed twice on the way up, at 140 and at 150.** This is the only limit in the service whose arrival is invisible to the people it affects: every other refusal answers somebody standing there and says so on the screen they are looking at, while this one fires in a queue worker against mail that arrives on its own. `capacity.js` sends both, to everyone with `owner` on the ACL.
    - **Both fire on a successful commit reaching an exact count, not on a refusal.** So the second one lands as the archive *becomes* full rather than after a letter has already been lost, and by the time anything can be refused the owners have been told twice. It also means no suppression row is needed: `commitPost` appends one letter at a time under an ETag, so every count from one upward is passed through exactly once and an equality test cannot repeat — two letters racing means one lands on 140 and the other retries onto 141.
    - **Deleting letters re-arms both, for free.** A once-ever row in `nudges` — the obvious implementation, and the one the other explanatory emails use — would have silently prevented that. The equality test gets it by construction.
    - **The full message names the thirty days out loud.** It is the single fact that decides whether this is urgent, and an owner who does not know it will reasonably assume a refused letter is either safe forever or already gone. Both messages also offer a reply as an alternative to deleting anything, because "delete your family's letters" is a poor only option.
    - **An owner who has opted out of all mail is not written to**, following `digest.js`: somebody who said stop has said something stronger than any preference of ours, and this is not urgent enough to be the exception that overrules them. The cost is real and accepted — that owner can lose letters silently.
    - **Sent from `runIngest`, not `commitLetter`**, which exempts promotion for the same reason the forward receipt is exempt: a claimed site replaying a backlog is being watched by the person who claimed it, and mail about how full it is getting would arrive while they are looking at the number.
  - **Promotion needs no exemption from the cap itself and gets the safe answer for free.** `promote.js` treats any status that is neither `stored` nor `duplicate` as a failure and leaves the pending blob in place, so a held backlog that runs into the ceiling is preserved rather than dropped.
  - ~~**Alerting is a log line and nothing more.**~~ **Alerting shipped 2026-08-06**, as `ingestCapAlert` in `main.bicep` — a scheduled query rule over `traces | where message has "daily cap reached"`, every fifteen minutes, threshold zero, into the same action group as the credential warnings.
    - **Threshold zero, and Sev1.** The cap is set far above the largest honest day, so a single occurrence is either a loop or a person having a very bad time, and both want a human. Unlike the credential warnings, which have thirty days of slack, this one is costing money for as long as it is ignored.
...
**Partly shipped, out of order.** Edit, hide and delete, the `/login` chooser, the `401` deep link, the signed-in state on the root page, the `memberships` index, the root archive list, **invitations and member management**, **profile editing**, **the in-site switcher and `403` handling**, **the operator role**, ~~and now **site deletion**~~ **and site deletion, by owners and by operators**, were all pulled forward — each because something already built was resting on it. ~~**Everything else is untouched:** no restore-original, no `/manage/last-received`.~~ **The phase is now complete.**
...
- **Site switcher** and **signed-in root redirect**, with the no-memberships explanation for an address that isn't on any ACL. See [Switching between sites](#switching-between-sites).
  - ~~**`lastPostAt` is declared but nothing keeps it current.**~~ **Fixed by moving it off the membership rows.** Both `lastPostAt` and the site's display name were copied onto every member's row, so keeping either current meant a cross-partition write per member, on the ingest path, per post. Neither was ever updated: the name froze at whatever the claimant first typed, and the sort silently became "most recently *added*". They now live on one row per site in a `sites` table, partitioned by slug. Ingest writes a single entity per post in the slug's own partition, and the read side pays a point read per archive the signed-in person belongs to.
  - **It is a separate table rather than a reserved partition inside `memberships`.** `rebuildMemberships` prunes by scanning for rows whose `rowKey` is the slug and whose partition is not on the ACL. A site row keyed that way matches that description exactly, so the repair path would have deleted it — silently, on the drift-recovery path that exists precisely because something has already gone wrong.
  - **The site writes are guarded; the letter is not.** A table failure during ingest is logged and swallowed, because this is a sort key and the alternative is making the sender's mail server retry and deliver the letter twice. The same guard covers the display-name write during a claim, after a test showed a failure there could fail the whole claim *after* the ACL had been created.
  - **The root half shipped, and it lists rather than redirects.** A signed-in visitor is offered their archives by name instead of being bounced to the most recent one: someone signed in with the wrong account needs to be able to *see* that, and a page that jumps straight through gives them no opportunity to notice. **The switcher shipped 2026-08-06** — a `details` disclosure in the site masthead, populated from `/api/memberships`, listing every archive except the one on screen; nothing is drawn for the majority who have one. It is fetched without being awaited and fails silently. **It also renders on the refusal page, with nothing excluded**, since somebody who has just been told no is exactly the person who has lost a URL.
...
- Owner admin view per [Editing and hiding posts](#editing-and-hiding-posts): edit any post's subject or body with edited HTML passing through the ingest sanitizer, ETag-guarded, dedup-derived fields read-only. Each edit stamps `editedBy` / `editedAt`, surfaced in the admin view and **stripped from the reader payload**. **Hidden posts are stripped server-side** in `/api/content/` and `/api/photo/` for `reader` callers and returned flagged to `owner` callers, rendered dimmed with an **Unhide** action. Hidden posts still participate in dedup.
  ...
  - ~~**Still outstanding from this bullet:** hidden posts are badged rather than dimmed, and there is no admin surface for `editedBy` / `editedAt` — they are written, and only readable in the blob.~~ **Both shipped 2026-08-06.** Hidden posts are dimmed with the reason in the summary line, and the owner bar names who last edited the letter and on what day — in the reader's own zone, unlike the letter dates, because this is a real instant rather than a day somebody wrote at the top of a page. Owner-only, still: telling readers a letter was edited would advertise the anonymisation it mostly exists to perform.
- ~~**Restore original** per [Restoring the original](#restoring-the-original): owner-only, one post at a time, re-runs render from `raw/` and overwrites the post. Confirmation names whose edits are being discarded; `editedBy` / `editedAt` clear and `hidden` is preserved.~~ **Shipped 2026-08-06** as `POST /api/posts/{slug}/{postId}/restore`, and offered only on a post somebody has edited. The decision recorded with it: pictures an owner added to a letter are discarded along with the edits, because the restore rebuilds `photos` from the message that arrived.
- **Site deletion** per [Post-mission archive](#post-mission-archive). **Shipped 2026-08-06.** `DELETE /api/site/{slug}`, owners only, with the typed archive name as confirmation; the `deletions` table holding one row per pending erase; and a nightly `erase` timer at 04:15 UTC that empties `raw/`, `rendered/`, `config/`, `exports/` and `pending/` under the slug. It runs as `mj-id-purge`, asked for explicitly by client ID per [Phase 0](#phase-0--foundation).
  ...
  - **Operator deletion is the same button with one more field in it.** ~~Still outstanding: operator-initiated deletion of somebody else's archive is only available through the owner path an operator already resolves into.~~ That *is* the design — one code path, one retention story, one confirmation — and what was actually missing was the thing that makes an operator's deletion different: the reason. `DELETE /api/site/{slug}` now refuses an operator who has not given one, and `/settings/{slug}` shows the amber operator banner and a required *Why are you deleting this?* box to a caller the gate flagged. `GET /api/profile/{slug}` carries `viaOperator` for that reason alone — the form cannot ask for something it does not know is required, and a second endpoint answering the same question is how two answers eventually differ.
    - **Owners are asked for nothing, deliberately**, including an operator deleting their own family's archive — they resolve through the ACL, so the flag is false. A family leaving is not made to justify itself, and a field that fires when nothing is wrong is the one people learn to fill in without reading.
    - **Present, not long.** A minimum length is theatre against somebody who can type `x`; the audience for the string is the operator's own future self, and the slug, the actor, the member count and the date are already on the record.
    - **The typed name is checked first.** Being told to explain a deletion you have not yet confirmed describes the wrong problem.
...
- **Invitations** per [Invitations](#invitations). **Shipped 2026-08-05, early**, because `hello@` was the only real dependency and an archive readable by exactly one person was the most conspicuous thing left.
  ...
  - **The cap counts issued rows, including revoked ones, which is why revocation is a tombstone.** A cap counting only surviving rows is reset by the revoke button, and invite/revoke/invite is then an unbounded loop. The tombstone also keeps a withdrawn token explicitly refused rather than merely unrecognized.
...
- ~~Owner-managed profile: display name, optional `returnDate`, and `alternateSenders` for missionaries writing from a personal account.~~ **Display name and `returnDate` shipped; `alternateSenders` deliberately not.** The name is set once during a claim and was, until now, permanent — a typo in the string that heads every page, titles the tab, labels the archive on the root list, and goes out in the subject line of every invitation, with no way to correct it. `/settings/{slug}` and `GET`/`PUT /api/profile/{slug}` fix that, owners only.
  - **`profile.json` is the record and the `sites` row is the index**, the same relationship `acl.json` has with `memberships`. Getting this backwards was a real bug: the rename before this one wrote only the table, and `tools/rebuild-memberships.js` restores the name *from the blob*, so a repair run would have silently reverted an owner's rename. Writing the blob first and mirroring second also means a failed mirror leaves the record correct and the index stale, which the repair tool fixes.
  - **`alternateSenders` is carried through untouched and given no control.** It is in the file, seeded empty, and nothing reads it — routing keys on `@missionary.org` alone. A field that appears to decide who may publish into an archive and in fact decides nothing is a lie with the shape of a security setting. The only property tested is that editing the name does not delete it.
...
- **Ownership-window UI** per [Ownership and the 60-day window](#ownership-and-the-60-day-window): ~~enforce `verifiedMissionary` removal protection~~ **(shipped with member removal — the flag is set only by the `claim@` path, and a pending redemption deliberately writes it `false`, because the protection it confers is too strong to hand out on that evidence)**. ~~Persistent banner while any owner is on `missionary.org`~~ and ~~a standing prompt to get the missionary claimed while their address still works~~ **are both dropped, and nothing replaces them.**
  - **The banner's condition can never be true.** Every path that creates an owner — claim redemption, relay redemption, invitation acceptance — records the *signed-in* identity, and a missionary cannot sign in as themselves: mailbox access runs through the Church's own sign-in and a link across into Gmail, so they have no Google credential to hand an OAuth consent screen, quite apart from whether the tenant permits third-party OAuth at all. The mailbox proves who they are; the personal account holds the role. Nothing ever writes a `missionary.org` address into `acl.json`, so the banner would render for nobody.
  - **Claiming is a failsafe, not the expected path.** The ordinary case is a parent who set the site up, runs it, and keeps running it; the missionary is off doing something else for two years and has no reason to want an admin page. `claim@` exists so that a site *can* be recovered when the person it belongs to needs it — a dispute, an absent parent, a missionary who wants their own letters. Prompting for it turns a rarely-needed escape hatch into a chore on somebody's screen, and the chore appears on every visit while the need appears almost never.
- **Operator authorization** per [Service operators](#service-operators): `OPERATOR_EMAILS` resolves to `owner` inside the shared authorization function from Phase 3, above the `acl.json` lookup and with no write to `acl.json` or `memberships`. Includes the `/manage/last-received` service-wide view, the "acting as operator" banner, `OperatorAction` telemetry on reads as well as writes, operator site deletion with a recorded reason, and `verifiedMissionary` removal blocked for operators exactly as for owners. The email path is untouched — forwarding rights stay `acl.json`-only.
  - **Shipped 2026-08-06**, minus ~~`/manage/last-received` and operator site deletion~~ **(both completed 2026-08-19 — see the reason field above and the flow view below)**. The `verifiedMissionary` protection already applies to operators for free, because an operator is an owner by the time `members.js` sees them and that rule refuses owners.
  - **`/manage/last-received` is the only view in the service that spans archives, which is the whole reason it exists.** Ingest stopping is silent by construction: no letter arrives, so no letter is missing from any page, so no owner has anything to report. The first evidence would be a family asking where last month went. It is a second table on the operator page, above the deletions, and `lib/flow.js` assembles it from indexes that already exist — the `sites` rows, the `pending` manifests, the `deletions` table — so nothing here is authoritative about anything and a wrong answer is a bug in a report rather than a bug in the service.
    - **Two dates, because they are two questions.** `lastPostAt` is the date the letter carries and is what the landing page sorts on; `lastReceivedAt` is when it landed here, and it is new. They diverge on the one path that matters: a family forwarding two years of backlog in an evening is the busiest that archive has ever been, and on `lastPostAt` alone it reads as eighteen months silent. **The gap between the columns is the most informative number on the page.**
    - **Rows written before arrivals were recorded stay blank.** Backfilling from `lastPostAt` would make the two columns agree by construction on exactly the archives with the longest history, which is where a real gap is likeliest. They still *rank* on the letter they have, so an archive with history does not sink below one that has never received anything.
    - **Sorted most recently active first, not longest silent first.** Longest-silent sounds right for a monitoring page and is not: a missionary who came home is silent forever and legitimately so, and within a year the top of that list would be nothing but finished archives with no way to tell them from a broken one.
    - **Pending and deleted archives are on it too, each named as what it is.** A deleted archive keeps its site row, so without a state column it sits there looking exactly like a working one — and this is the page somebody goes to to be told otherwise. A pending slug ranks alongside the rest on the date its letters arrived rather than sinking to the bottom, because it is the row about a family nobody has met and the likeliest thing to be wrong.
    - **The held-letter count comes from the blobs, not from `messageCount`.** That field counts everything ever held and promotion never decrements it, so a claimed archive would report a dozen letters stuck in limbo forever. Against a *live* archive a nonzero count is an anomaly rather than a status: promotion writes into `raw/` and only then deletes the held copy, so anything still there is a promotion that failed partway and left the only copy of somebody's mail in a container nothing reads. It earns a column precisely because it is ordinarily always zero.
    - **Erasure now forgets the site row, and deletion still does not.** The row carries the missionary's display name. Deletion keeps it because thirty days is a window and restoring an archive whose name had been thrown away would put a family back with a blank masthead; erasure is the promise that nothing is left, and a row surviving it would keep the archive on this page forever with nothing behind it.
    - **Not paged and not filtered.** Partial answers are what make a monitoring page untrustworthy, and the whole point of looking is to see whether something is missing. It is one scan of a table holding one row per archive, read by one or two people, occasionally.
  ...
  - **An operator who is only a *reader* on a site still gets `owner`, and is still flagged.** Their own family's archive, where they are an ACL owner, is not flagged at all: a warning that fires when nothing is wrong is one people learn to stop reading.
  - **The audit event is emitted from inside the two gates, not from each endpoint**, so every route that authorizes through `gate`/`siteGate` is covered by construction. It is `log.warn('OperatorAction', …)` with actor, slug, method, route and timestamp — `warn` because nothing here takes a dependency on the App Insights SDK, so it lands in `traces` and severity is the only thing separating it from the chatter. **The route is logged without its query string.**
  - **`viaOperator` went into the `posts.json` ETag salt.** The salt already carried the role, because owners and readers get different bodies; the flag changes the body too, and `Cache-Control: private, no-cache` means every load is a revalidation that reuses the cached body on a 304. Without the salt, an operator removed from a family's ACL would keep revalidating into a copy with the warning switched off. Salting with `false` is a no-op string, so no existing browser cache was invalidated.
  - **Not memoized.** A cached copy would mean an operator removed from the list keeps their access until the host happens to recycle — the moment the removal matters most.
  - **Unset means nobody**, tested from that direction explicitly. An empty or whitespace setting produces an empty set, never a wildcard. The parse accepts commas, semicolons and whitespace because the value is typed into a portal text box by a human.
  - **The parameter is restated in `main.bicepparam` rather than left to its default**, for the reason learned with `MAIL_ALLOWLIST`: `what-if` cannot see `siteConfig.appSettings`, so an omitted parameter blanks the setting on the next deployment. Here that direction is safe — it revokes rather than grants — but it would revoke silently.
- ~~**Credential expiry announces itself.**~~ **Shipped.** Both credentials that have a date carry it in Key Vault — `aad-client-secret` to **2028-08-04**, `cloudflare-api-token` to **2027-08-31** — and an Event Grid system topic on the vault routes `SecretNearExpiry` and `SecretExpired` to an action group that mails the operator. Thirty days' warning, Sev2.
  - **This is the failure mode the whole thing exists for.** The Entra secret would fail the way the missing ID-token grant did — silently, at the callback, with Google still working. The Cloudflare send token is quieter still: sends return `10101 unauthorized`, which the mailer logs and swallows by design, so nothing breaks and claim emails simply stop reaching anybody. The first evidence would be a pending site expiring unoffered, an alert arriving sixty days after the cause.
  - **The alert must not travel through our own mailer.** Every other message this service sends goes out through Cloudflare using `cloudflare-api-token` — one of the secrets being watched. Routing the warning through the mailer would mean the alert that the sending credential is expiring is itself sent with the sending credential, so the first alert that mattered would be the first that could not be delivered. Event Grid's `MonitorAlert` destination shares nothing with the system it reports on, and needs no code.
  - **`MonitorAlert` requires `eventDeliverySchema: 'CloudEventSchemaV1_0'`,** which is not the default. Omitting it fails the whole template with a message that names the schema but not the resource asking for it. `Microsoft.EventGrid` also has to be registered on the subscription first, and `az eventgrid ... show` still cannot read the result — it pins an API version older than the feature, so verification goes through `az resource show --api-version 2025-02-15`.
  - **Only two event types.** The vault also emits `SecretNewVersionCreated` on every write, which would turn a rotation — the fix — into an alert of its own.
  ...
  - **Key Vault cannot renew any of these.** Auto-renewal is certificates-only for integrated CAs; it has no way to ask Entra or Google for a fresh client secret. The alert is the whole mechanism.
...
- **The printer gets our URL, never a SAS.** Their terms say they keep files to make reprints, so the link has to work in three years — and a three-year SAS is a bearer token for a blob sitting in somebody else's database that cannot be withdrawn without rotating a key everything depends on. `print/{slug}/{id}/letters.pdf` carries a token signed with the same key the invitation links use, names both the site and the book inside the signature, and redirects to an hour-long SAS minted at the moment of the fetch.
- **Test environment by default in code, production by name in Bicep.** `PEECHO_BASE` falls back to `https://test.www.peecho.com` when nothing sets it, so a developer running this with no configuration cannot post a book to anybody; `main.bicep` names production, because that file *is* production and the choice belongs somewhere reviewable rather than in a script somebody ran once. Using the test environment for real means registering there too — the live merchant key is not recognized in it.
- **The account block was a false alarm, and an expensive one.** Peecho's Company details form demands a VAT number a US sole trader does not have, and the API answered `APP_NO_COMP_DETAILS`, so this was written down as blocked. It is not: the API wants **Billing Information and Billing Address**, neither of which asks for VAT, and once those were filled in a create-publication call returned a real secure publication id and a working checkout page. Saving the webhook URLs does insist on Company details, and that field turned out to accept `1` — a length check, not a registry lookup. Recorded in [printing.md](printing.md) so the next person does not spend an evening on a form nothing reads.
- **The button makes a link rather than a redirect.** Peecho's secure checkout is good for two months and works for anybody holding it, so what comes back is shown and left on screen: a grandmother buys her own copy from a link she was sent, without an account here and without the owner ordering on her behalf. It is hidden entirely where `PEECHO_API_KEY` does not resolve — the status endpoint says whether there is a printer — because a control whose only possible answer is "printing is not switched on yet" is a control that exists to apologize.
...
- **No dependency and no bundler.** The CSP is `script-src 'self'`, and a vendored rich-text library is a large amount of surface for what owners actually do: fix a typo, cut a paragraph, remove a name, delete a photo. **An earlier version of this bullet also cited the offline archive, and that was simply wrong** — the owner controls render only when `mount()` is given an `admin` object, as the bullet two sections up says, so the editor is never in the zip and `file://` never bound it. The conclusion survives on the other two reasons. The argument did not, and hand-binding the formatting shortcuts was paid for partly by a constraint that did not apply.
...
**Built.** `POST /api/posts/{slug}/{postId}/photos` and `DELETE /api/posts/{slug}/{postId}/photos/{photoId}`, with an `Add pictures` button in the owner bar and a small remove control on the tiles an owner put there.
...
- **The upload is raw bytes with a `Content-Type` header, one picture per request.** No multipart, because there is nothing to multi-part — there are no other fields, and parsing a format built for forms in order to carry one file would be the only place in the service that needed a body parser. Several pictures are several requests, each its own commit, and the page is reloaded once at the end. The first failure stops the run and says how many got through, because "three of five" is the only honest thing to say. That sentence has to outlive the page that wrote it — the reload is the only way to show the pictures that did land, and it takes the status line with it — so it is stashed in `sessionStorage` keyed by archive and letter, and the letter's own controls read it back and clear it as they are drawn.
...
- **They sit in the same album as the unreferenced attachments.** No separate grouping and no heading. `renderAlbum` draws every photo the letter does not display inline, in the order `present.js` hands them over — see [Pictures are shown in the order they were taken](#pictures-are-shown-in-the-order-they-were-taken) — while a group of their own would draw a line through the letter's pictures on behalf of a distinction the reader has no use for.
- **And a letter that ends in a burst of photographs draws one row, not two.** The bullet above holds for pictures that were never in the body, and the case it does not reach is the common one: a missionary who pastes five photos at the foot of her letter and an owner who adds a sixth afterwards. The five are inline, so they render as the burst row [item 4](#reader-ui-backlog) built; the sixth is not, so it renders as the album; and the two sit stacked, alike in every particular, on behalf of a difference in a mail client from two years ago. `reader.js` moves a trailing run into the album at display time — `mergeTrailingRun`, no data touched, no ingest changed.
  - **Only a run at the very end moves, and only a run.** A burst in the middle of a letter is where the missionary put it, and pulling it down to the foot would rearrange the letter rather than tidy it. A single closing photograph is not a burst either: it is an illustration, it is already presented as one, and shrinking it to a thumbnail to fill out a row would be the same rearrangement in miniature.
  - **Into the album rather than the other way about**, because the album is where an owner's remove controls live. The moved frames carry `data-inline` so they are not offered one, since a picture in the letter's own body is deleted by editing the letter.
  - **It has to be undone before an edit, and that is the part that could lose a letter.** `open()` hands the body to the browser as an editable region and `markup()` writes back whatever is in it, so a letter whose own pictures were sitting in the album would be saved without them. `splitTrailingRun` puts them back first and `discard()` re-merges on cancel. The pair has to stay symmetric.
  - **Measured before it was built:** across all four archives, 90 letters carry photographs and exactly 2 were drawing both rows. That is small enough to look like a non-problem and is not — every one of Isaac's 42 letters with inline photographs would join them the first time anybody added a picture to one.
...
- **There is no raw original to fall back on.** Every other photo in the system has an untouched copy inside the `.eml` in `raw/`, which is what makes [reprocessing](#photo-handling) and full-resolution download possible. An upload arrives with no message around it — so it is not reprocessable, and the design says so rather than storing a second original to pretend otherwise. A re-render carries these entries across untouched; the renditions are already in `rendered/` and nothing in the pipeline deletes a blob.
- **Owner-only, capped at 48 per letter, 20 MB per picture — and ~~`ETag`-guarded~~ deliberately not.** Every other write here replaces a document the browser composed from a copy of the site, which is why they carry the staleness claim. These two do not: one appends an entry whose id is the hash of the bytes, the other names a single id to drop. Neither can undo an edit it never saw, and enforcing it would break the ordinary case of adding two pictures in a row — the first moves the `ETag` the second was holding.
- **The 20 MB is checked twice: once on `Content-Length`, once on the bytes.** The header is a claim and not a promise, so it can never be the only check — but when it is present and honest it saves buffering twenty megabytes in order to throw them away. Both photo uploads and the book's cover picture used to read the whole body first and measure it afterwards; `fetchPicked` had done it the right way round since it was written, and `overSizeClaim` in `photos.js` is now the one shared answer. The platform backstop underneath all of it is Static Web Apps' own 30 MB request limit, which is what makes "buffer first" a tidiness problem rather than a hole.
- **The cap is 48 and the number comes from the printed book.** The album prints six photographs to a page, so 48 is eight pages of plates behind a letter that usually runs to two — ten leaves for one fortnight, which is where a letter stops being a letter with pictures and becomes an album with a caption. It lives in `photos.js` as `MAX_PHOTOS` and is sent to the browser in the content payload rather than written down twice. **It was 24**, and briefly 200 while the bulk spread below was being built.
- **It counts every picture on the letter, from any source.** It used to count only what an owner had added, on the reasoning that a forward arriving with thirty attachments should not lock its own owner out. But the book counts them together, so the cap has to. Measured against one real archive at this cap, one letter out of forty-four is over and two photographs are refused, which is a sentence an owner can act on.
- **Ingest is not capped the way adding is, and the difference is the point.** A letter that arrives carrying forty attachments keeps all forty: they are photographs a missionary actually sent, the render path builds `photos` straight from the `.eml` and never goes near `attachPhoto`, and refusing there would discard them with nobody to tell. The rule is that the cap governs *adding*, not *arriving* — such a letter is simply full, and takes no more by hand. It costs the book seven album pages, which is fine. For scale: one complete real archive carries fifteen attached photographs across four letters, and its fullest single letter carries five.
  - **There is nevertheless a ceiling at ingest, at the same 48.** Not a policy about photographs but a bound on work: `MAX_RAW_BYTES` limits a message to 26 MiB and says nothing about how many pictures are inside it, and 26 MiB is tens of thousands of distinct small images, each one a transcode and two blob writes. That render would exhaust the thirty-minute timeout, be redelivered, and exhaust it four more times before the letter reached the poison queue. `renderPhotos` stops at 48 and logs at error level.
  - **The honest cost is that a letter arriving with more than 48 photographs loses the remainder,** which is the thing the bullet above says not to do. It is accepted because the measured distance to it is large — the fullest real letter carried five — and because the alternative is an unbounded loop over attacker-chosen input. `raw/` still holds every one of them, so the decision is reversible by re-rendering if the number ever turns out to be wrong.
- **A whole selection is checked in the browser before any of it is sent.** The server still refuses one picture at a time and that is the check that counts, but a `409` arriving at picture 380 of a six-hundred-file run stops it and leaves the owner to work out which letter filled up. The same sentence said first names the letter and the room left in it, and uploads nothing.
- **One sitting carries 30 pictures, which is about a minute.** Measured: a photograph off a phone averages 1.5 MB and takes roughly two seconds to upload, transcode into both renditions and commit. Unlike the per-letter cap this is nobody's rule but ours and it is about the run rather than the archive — a ten-minute run is one that gets interrupted by a closed laptop, and every picture after the interruption has to be found again in a folder of six hundred. Six hundred photographs is therefore twenty sittings, and the spread still works inside each one, so a batch picked at random still lands on the right letters.
- **The status line counts both halves of the run.** Reading the dates is a per-file read of the first 128 KB and is counted out loud, because on a phone it is long enough to look like nothing is happening; the uploads are counted the same way. `reader.js` animates any message ending in an ellipsis and keeps the same dots element across updates, so a ticking count does not restart them.
- **Adding is not part of the edit mode**, and sits beside `Edit` rather than inside it. Everything in that mode is one save of one document, which either sticks or does not; an upload is several round trips that each either stick or do not, and mixing them would mean `Cancel` undoing some of what just happened and none of the rest.
- **Removal is offered, and only for pictures with an `addedAt`.** Without it a misclicked upload is unremovable: `Restore original` only appears once a post has been edited, and adding a picture is not an edit. A picture that came with the letter stays under that button, which asks first and says what it is about to discard.
...
**Built.** Choosing several pictures at once offers to place each one on the letter it was taken before, instead of putting the whole pile on whichever letter the owner happened to be standing on.

This is the returning-missionary case, and it is not a nicety. A completed archive is around forty letters; the phone that comes home with it holds several hundred photographs, almost none of which were ever emailed. Placing them by hand means opening a letter, remembering what fortnight it covered, picking the right dozen out of a folder sorted by filename, and doing that forty times. Nobody does that, so the pictures stay in the folder and the archive stays thin.

- **The date is read in the browser, off the file, before it is sent.** There is nowhere else it can come from. The transcoder strips EXIF deliberately, because it carries GPS and a missionary's coordinates are not the family's to publish, and no copy of the original bytes is kept — an upload has no `.eml` in `raw/` behind it. A date not read in `web/taken.js` is a date that does not exist.
- **EXIF first, the filename second, and the filename is not a guess.** `DateTimeOriginal` is what a phone's own camera writes. WhatsApp strips the metadata and names the file after the day; the Android camera, Messenger and the screenshot tool all do something similar. Measured across one real mission's photographs, on the 236 files carrying both a filename stamp and EXIF, the two agreed every time. Roughly half the pile has no EXIF at all and would otherwise be undatable.
- **The refusals are the part that matters.** Message attachments are named after enormous ids — `received_1206313707238074` — and a pattern that matched part of a longer digit run would read the first eight of those as a date, file the photograph under the year 1206, and land it on the first letter in the archive. Every pattern is anchored against neighbouring digits, the year is bounded, and the date is round-tripped so that February 31st is refused rather than rolled into March.
- **A picture goes on the first letter written on or after it was taken.** That is how the letters were written — something happened on the Wednesday and was described on the next P-day — so the picture lands beside the paragraph about it. One taken after the last letter goes on the last letter rather than being refused; the mission is over, and there will be no letter after it.
- **The owner is asked once, and only where there is a choice to make.** One picture, or a pile carrying no dates, or an archive with a single letter in it, goes where the owner was standing without a dialog. So does a pile that all belongs to the letter in front of them. The question names how many carry a date, how many letters they would reach, and how many said nothing and would stay put — and the third button is Cancel.
- **The hint is a header, and a bad one is ignored rather than refused.** `X-Taken-At`, a wall clock with no offset on it, matching `originalDate` for the same reason: EXIF records the local time wherever the shutter was pressed and no zone at all, so there is no instant to be had without inventing one. The server checks the shape and drops anything else in silence — losing the order of one photograph is small, and failing an upload in the middle of a run of six hundred is not.

### Pictures are shown in the order they were taken

**Built.** `present.js` sorts each letter's `photos` by `takenAt` on the way out, which is the one place that covers the letter strip, the site album and the offline export at once.

Arrival order was never an order. It is the order a file picker happened to hand things over in, and for a bulk add it is a record of how the work was done rather than of anything that happened in the field.

- **Only owner-added pictures carry a `takenAt`.** The date is read in the browser and a letter's own attachments never pass through one. So those keep the order the missionary attached them in — which is the order they were meant to be seen in — and the sort has to leave it alone.
- **An absent date sorts first and stable.** Undated pictures share an empty key, `Array.prototype.sort` is stable, and a comparator returning zero pins them where they were. A letter's own pictures therefore lead, in the order they arrived, with anything added afterwards behind them in capture order.
- **The array is copied before sorting.** `pick` hands out the stored reference, and sorting in place would reorder the posts the render path is about to write back.
...
- **The cap is told to Google, not only enforced on the way back.** `MAX_PICK` is 24 — half the per-letter cap, because a picking session is hand-chosen and every pick costs a fetch from Google — so somebody who selects their whole camera roll is stopped at the picking screen rather than after a long wait and a refusal.
...
**Built.** A `Photo Album` button in the archive toolbar opens every photograph in the archive as one scrolling wall of thumbnails, grouped under the letter each came from, with a viewer over the top of it and a way back into that letter.
...
- **It is not in the downloaded zip**, which is a deliberate departure from the word cloud's *offline-if-free* rule. Video, if it ever lands, will live here. See [the zip is a lesser experience](#phase-5--offline-archive-export).
- **A grid, and it replaced a slideshow.** The first build was a deck of cards driven by Swiper, and it failed the way that design has to fail: it built a slide for every photograph in the archive up front, held the full-size image of every one it had passed, and started a timer the moment it opened — so on a cold connection it spent that timer moving past pictures that had not arrived. Measured at six hundred photographs it was holding 109 full-size images after a hundred slides and dropping six frames in a hundred and thirty-nine while being dragged. The grid asks for the thumbnails on screen and nothing else, through the browser's own `loading="lazy"`, which needs no window to keep track of and no library at all. Retiring it took a 149K bundle out of the site.
- **The slideshow survives, in the viewer and on request.** Clicking a thumbnail opens that one photograph; `Play` advances from there. The clock does not start until the picture on screen has decoded, so a slow connection makes the slideshow wait rather than run on empty — which is the specific fault that made the old one look broken rather than merely slow. Behind each photograph, blurred, is its own thumbnail, already in the cache from the grid, so there is something to look at from the first frame.
- **Back leaves the album, not the archive.** Both dialogs cover the window, so they read as places, and Back is what people reach for to leave a place — reported from the site, by the owner reaching for it himself. Untouched, one press throws away the archive underneath. Each layer now pushes a history entry as it opens and takes it back as it closes, whichever way it was closed, so Back goes up exactly one: from a photograph to the grid, from the grid to the letters. The URL never moves. What is recorded is depth, not an address, and an album nobody can link to does not belong in anyone's address bar. The reciprocal half is the part that bites: a layer closed by the `×` has to withdraw its own entry, or Back is later spent on a dialog that has already gone and appears to have stopped working. Closing two layers at once — which *Go to this letter* does — must be one `history.go(-2)` rather than two `back()` calls, because a browser is free to collapse two traversals queued in the same task, and does.
...
1. ~~**There is no way to contact a human.**~~ **Answered: `hello@pdayletters.com` is monitored and appears on the FAQ, the ask page, the opt-out pages and each site page.** Every path in this document that ends in "a service operator decides" — an abuse report, an ownership dispute after the [60-day cliff](#the-60-day-cliff), a missionary who finds a site about themselves — depends on it.

2. **~~Is Cloudflare's outbound header allowlist compatible with the design?~~ Answered: yes, from documentation.** All five headers Phase 8 needs are allowlisted — `In-Reply-To` and `References` under threading, `Auto-Submitted` under automated-message identification, and `List-Unsubscribe` / `List-Unsubscribe-Post` under list management. Two constraints come with them: `List-Unsubscribe` must carry an angle-bracketed `https:` or `mailto:` URI and plain HTTP is refused, and `List-Unsubscribe-Post` must be exactly `List-Unsubscribe=One-Click`, case-sensitive.

3. **~~Does `message.reply()` cover the ack path?~~ Answered: yes, with limits.** `reply()` requires the incoming message to pass DMARC, allows one reply per event, requires the reply recipient to match the incoming sender and the sending domain to match the receiving domain, and refuses a message carrying more than 100 `References` entries. Acks to a relative forwarding from an old ISP account fall back to a normal send and lose their threading.

4. **~~`mailauth` carries four high-severity advisories with no fix available.~~ Answered: fixed, and the baseline is empty.** For a long while the answer was "not reachable" — requiring `mailauth/lib/dkim/verify` pulls in 109 modules, of which `undici` accounted for zero and `nodemailer` for exactly one, `lib/addressparser/index.js`, a pure parser. Twenty-four findings stood against the tree, seventeen of them acknowledged in `functions/audit-baseline.json` with a written reason each. **Then they became fixable, so they were fixed.** `mailauth` 5 dropped `undici` 7 for `undici` 8, `sharp` moved to 0.35.4, and `overrides` in `functions/package.json` lift the `joi` and `nodemailer` versions `mailauth` pins to ones with the fixes in. `npm audit` reports zero, all 19 DKIM tests still pass, and the baseline is now empty. **A fix beats a note explaining why a fix is not needed.** The mechanism stays, because the case it was built for — a real advisory against code nothing loads, with no version to move to — will come round again. `functions/tools/audit-check.js` compares advisory *IDs* rather than counting them, since a count holds steady while one advisory is fixed and another appears, and each entry has to carry the reason it is unreachable: an entry without a reason is a silenced alarm.
