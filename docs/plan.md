...
2. **Does the message the Worker sees already carry Cloudflare's verdicts?** The format itself is no longer a question. Measured from the captures, Cloudflare stamps `Authentication-Results: mx.cloudflare.net; dkim=pass header.d=missionary.org header.s=google header.b=…; dmarc=pass header.from=missionary.org policy.dmarc=quarantine; spf=none (…) smtp.helo=…; spf=pass (…)`, and seals the same verdicts into `ARC-Authentication-Results: i=2`. Two traps for the parser: **there are two `spf=` results**, one for the HELO identity and one for the envelope sender, so a first-match read reports `none` for a message that passed; and in any capture taken out of a forwarded mailbox the *topmost* `Authentication-Results` belongs to the receiving provider (`mx.google.com`), not to Cloudflare. What remains unknown is whether Cloudflare has attached its header by the time the Email Worker runs or only on delivery — the fixtures cannot answer that, because every one of them was captured after delivery. If it has not, the ingest Function computes the verdicts itself with `mailauth`, which is already proven against these specimens.

3. **How does the ingest queue message need to be encoded?** The Worker posts the ULID as plain text inside the Put Message XML envelope. The Azure Storage SDKs base64-encode by default and the Functions host has historically accepted both, so this is likely a non-issue — but it is the kind of thing that presents as an unexplained mangled payload. Confirm the first time a message flows end to end, and base64 the body in the Worker if the trigger disagrees.

4. **Is Cloudflare's outbound header allowlist compatible with the design?** Custom headers on sent mail are allowlist-controlled (`E_HEADER_NOT_ALLOWED`). Phase 8 needs `In-Reply-To`, `References`, `Auto-Submitted`, `List-Unsubscribe`, and `List-Unsubscribe-Post`. If any are blocked, reply threading or one-click unsubscribe has to move to another provider. **Verify before Phase 8, not during it** — this is the single fact that decides whether Cloudflare can be the outbound provider at all.

5. **Does `message.reply()` cover the ack path?** It requires the *incoming* message to pass DMARC. `claim@` traffic does by definition, but an ack to a relative forwarding from an old ISP account may not qualify, in which case those acks fall back to a normal send and lose their threading.
