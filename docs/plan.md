...
- **Test environment by default.** `PEECHO_BASE` is `https://test.www.peecho.com` unless production is named explicitly, because the cost of getting that default the wrong way round is a real book, really printed, really shipped. Using it means registering there too — the live merchant key is not recognised in the test environment.
...
- **The keys are provisioned out of band**, by `infra/provision-peecho.ps1`, for the reason `provision-claim.ps1` exists: a Bicep-declared secret takes its value as a parameter, and parameters are kept in the deployment history in plain text. Bicep declares the four settings and the two Key Vault references; the script writes the secrets and prints the two webhook URLs, which have to be typed into their dashboard by hand because those are their calls to us.

**Next:** `order.product.thumbnail`, whose absence leaves the configurator with no preview and a `thumbnailUrl Required` in its console — the cover exists as a drawing in the PDF and would have to be rendered again as an image, which is the whole of the work. Then the line in pdayletters.com's own terms backing the IP and portrait-rights warranty Peecho's seller terms require.
