...
- **The DKIM measurements are written down** in [docs/dkim-findings.md](dkim-findings.md) rather than living in commit messages. Every table there is asserted by `functions/tests/dkim.test.js` and `functions/tests/arc.test.js`, so a client changing its behaviour surfaces as a test failure instead of a silent loss of coverage.
...
- ~~**De-duplication**, which promotion cannot work without.~~ **Already built.** `findDuplicate` takes a candidate and a list, so promotion gets batch behaviour for free by committing letters one at a time against an accumulating `posts.json` — the same loop, the same ETag, the same gate. The batch dedup this bullet called for did not need to be written.
...
- **The gate is not the preference.** `postAckEmails` / `dedupeAckEmails` on a `users` row are still unbuilt, because the settings page that would set them is unbuilt and a preference nobody can change is not a preference. The daily gate and the existing opt-out stand in until then — and the opt-out is honoured here, since a forwarder who asked us to stop is exactly the person a well-meant receipt would annoy most.
...
- ~~**Suppression has to be visible to owners, not just to us.**~~ **Shipped 2026-08-06**, as `delivery.js` and the `deliveries` table, keyed by SHA-256 of the address exactly as `optouts` is.
  ...
  - **On the page it dims rather than badges**, the same 0.55 as a hidden letter, with one sentence on the line in ordinary words. **The controls stay at full contrast** — Resend is often the next thing to press, and dimming the remedy along with the problem would be the wrong half. The colour is warm rather than red, because an alarming colour beside a grandmother's address reads as an accusation.
- **`Message-ID` cannot be set on our own mail** — it is platform-controlled and generated on a Cloudflare domain. Threading still works, because `In-Reply-To` and `References` are ours to write, but nothing downstream may assume it can recognise its own outbound `Message-ID` later. This is also why an *inbound* `Message-ID` is worth storing: it is the only handle any of this ever gets.
...
- **Six colours, picked off a hash of the word rather than at random**, so a word keeps its colour when a resize repacks the cloud. They are the site's accent plus five siblings, pulled toward the ink rather than being true pastels, because a word can be 11px and nothing pale reads at that size.
