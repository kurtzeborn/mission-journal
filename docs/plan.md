...
An owner types addresses; the people behind them have to find out somehow. Until now the plan stopped at *"that address is added to `acl.json`"* — which is a database write, not an invitation. **Nobody was ever told.** Access materialized only if the owner separately texted them a URL, so the invite button did nothing the invitee could observe, and the likeliest outcome was a parent adding twelve relatives and reasonably assuming that was the job.
...
It exists to defuse what would otherwise be the most common support question in the service. The ACL is keyed on email address, but **the address an owner knows for someone is frequently not the address behind their Google or Microsoft account.** A parent invites `grandma@aol.com`; Grandma signs in with the Gmail account her tablet set up years ago; the ACL check fails; she is locked out of a site she was just invited to, and neither she nor the parent can see why. Multiply by every relative with a work address, an old ISP address, or a shared household mailbox.

Binding on acceptance dissolves the entire class: whatever identity signs in *through the invitation link* is written to the ACL. The typed address becomes **where to send the invitation**, not **who the person must prove they are**. Both are recorded — `invitedEmail` alongside the bound identity — so the owner's list still shows the address they typed.

**The security trade is small and already accepted elsewhere.** A forwarded invitation email lets the recipient in. That is precisely the property the claim link has, and the claim link grants ownership of the whole site; an invitation grants read access to letters the forwarder could already read. Single-use binding caps exposure at one identity, and an owner can revoke.

**ACL entries therefore carry an acceptance state** — `invited` (no identity bound yet) or `active` — surfaced in the owner's admin list, because it answers the question that is otherwise unanswerable: *"Grandma says she still can't see it."* The owner looks, sees the invitation was never accepted, and resends, which issues a fresh token and invalidates the old one. **A resend is owner-initiated and manual**, which is what keeps "never repeated" true: the service never decides on its own to email an invitee a second time.

#### Signed in, but not on the list

A valid session on a site you don't belong to is a `403`, and it is a completely different situation from the expired-session `401` above: signing in again does not help, because the problem isn't *that* you're signed in but *which* account you're signed in as. It will happen constantly — the mismatched-address case above, a revoked reader following an old bookmark, someone opening a link that got forwarded around a ward.

Unhandled, all three get a blank error page. Instead, serve a page that names the identity being rejected and what to do about it: *"You're signed in as `grandma.smith@gmail.com`, but that address doesn't have access to Elder Smith's letters. If you were invited at a different address, sign out and sign in with that one — or ask whoever invited you to add this address."* A **Sign out and try another account** button points at `/.auth/logout` with a redirect back to the same path.

Naming the signed-in address is the whole point. Without it, the user cannot distinguish "wrong account" from "not invited," and those have opposite remedies. The missionary's display name appearing here to an unauthorized visitor is a deliberate and very small disclosure — they already hold the slug, which is the missionary's own email local-part, so the name is not news.
...
5. **Claim.** The link opens `pdayletters.com/claim/{token}`, and that page is **anonymous-allowed on purpose.** An auth-gated claim route throws a Google consent screen at someone whose only context is an email they just opened — alarming, and it explains nothing. So the page leads with what is being claimed: the missionary's name, the counts (*"7 letters and 31 photos from Elder Smith are waiting"*), a few subject lines and dates so it is self-evidently the right person, and a plain statement of what happens next — *you'll sign in, and you'll be the person who runs this site.* Only then the **Sign in with Google / Sign in with Microsoft** pair, each threading `post_login_redirect_uri` back to this same `/claim/{token}` URL so the token survives the OAuth round trip.
   ...
   **An expired or already-used link must not be a dead end.** The token's lifetime has to match the pending site's rather than undercut it — a fixed 14-day token against a 60-day retention window means a parent who digs the email out on day 20 is told the link is dead while all seven letters sit there intact. So the token **expires with the pending site**, sharing its rolling `expiresAt`, and lives exactly as long as there is something to claim. The failure page still does something useful: if the site is now active, *"This site is already set up"* plus a sign-in link, letting the ACL decide; if the pending site has purged, say so plainly; if the token is merely stale or spent, offer **"Email me a new link"** — which sends only to an address already listed in `claim.json` as previously emailed, so the button cannot be used to mail anyone new.
