...
- **`main.bicep` described the infrastructure for months without ever deploying it, and first ran on 2026-08-05.** Every other component has a deployment path — `deploy-web.yml` ships the static site on a push to `web/**`, `deploy-worker.yml` ships the Worker, and the Function App code is published by hand with `func azure functionapp publish`. The template had none, so nothing ever forced it to be true, and it accumulated changes made directly against the live account. Infrastructure as code that is never applied is not infrastructure as code; it is a second description of the system, written in a language that makes it look authoritative.
  ...
  **The remaining gap is that the deployment is still manual, which is what caused the drift.** A workflow would close it, and needs a decision first: it requires Azure credentials in GitHub, which reopens the federated-credential question deliberately left alone. Until that is settled, the reconciliation holds only as long as someone keeps running it.
