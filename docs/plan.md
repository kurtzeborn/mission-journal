...
**Partly shipped, out of order.** Edit, hide and delete, the `/login` chooser, the `401` deep link, the signed-in state on the root page, the `memberships` index, the root archive list, **invitations and member management**, and now **profile editing** were all pulled forward — each because something already built was resting on it. **Everything else is untouched:** no site switcher inside a site, no `403` page, no restore-original, no site deletion, no operator role.
