...
- **No dependency can change without a commit, and that was measured rather than assumed.** Both `package.json` files carry committed lock files, and both workflows that install run `npm ci`. The uncertain path was the Function App, which deploys by hand with `func azure functionapp publish --build remote` — nobody writes `npm ci` there, so whether Oryx honours `package-lock.json` is a property of someone else's build system.
