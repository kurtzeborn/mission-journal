...
- **Credential expiry has to announce itself.** The Entra client secret behind Microsoft sign-in expires **2028-08-04** and will fail the way the missing ID-token grant did — silently, at the callback, with Google still working. Key Vault carries the real date on `aad-client-secret`, so the remaining work is to route Key Vault's `SecretNearExpiry` Event Grid event (fired 30 days out) to a notification. **The Cloudflare send token (`cloudflare-api-token`) expires sooner — 2027-08-31 — and its failure is quieter still:** sends return `10101 unauthorized`, which the mailer logs and swallows by design, so nothing breaks and claim emails simply stop reaching anybody. The first evidence would be a pending site expiring unoffered — an alert arriving sixty days after the cause. **Set the expiry date on that Key Vault secret.** Three things measured while setting this up:
  ...
  - **Key Vault cannot renew either of these.** Auto-renewal is certificates-only for integrated CAs; it has no way to ask Entra or Google for a fresh client secret.
  - **`google-client-secret` deliberately carries no date**, because Google OAuth client secrets do not expire. Setting one would manufacture a false alarm.
