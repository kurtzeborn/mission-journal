...
**What remains:** the claim email is drafted but nothing sends it, which blocks the reminder and the tapering invitation series and makes `claimEmailSentAt` a field nothing writes in anger. The `claim@` handler, the "a site already exists" reply, the "email me a new link" path, and the timer-triggered expiry purge are all untouched. Every one of them is waiting on [Phase 8](#phase-8--outbound-mail-and-preferences), not on this phase.
...
- **`claim@` handler** per [Ownership and the 60-day window](#ownership-and-the-60-day-window): accept only DMARC-passing senders in `MISSIONARY_DOMAINS`, ignore everything else without reply, mail back a signed claim link that adds a `verifiedMissionary` owner. Reply copy must tell them to sign in with a **personal** Google/Microsoft account and explain the 60-day expiry. **Not started — and note that the shipped claim flow deliberately sets `verifiedMissionary: false`.** Following a link out of a forwarded email proves you were sent the link; it does not prove you are the missionary, and the removal protection that flag confers is too strong to hand out on that evidence.
...
**Not started, but no longer undecided.** The provider is **Cloudflare Email Service** — what was Email Routing has become a full sending product, with a `send_email` Workers binding, a REST API, SMTP, a suppression list, and Google Postmaster feedback. Every header this phase needs is allowlisted, so nothing here has to move elsewhere. The service still cannot send anything, and two prerequisites stand between it and being able to:
...
- **Email preview is on by default for a new sending domain and must be off before the first claim email.** It retains full message content for about seven days in the activity log, and a claim email's entire security model is that its token is seen only by the person who received it.
...
**Partly shipped, out of order.** Edit, hide and delete, the `/login` chooser, the `401` deep link, the signed-in state on the root page, and now the `memberships` index and the root archive list were all pulled forward into Stage 1 — each because something already built was resting on it. **Everything else is untouched:** no site switcher inside a site, no `403` page, no restore-original, no site deletion, no invitations, no profile editing, no operator role.
...
- **Site switcher** and **signed-in root redirect**, with the no-memberships explanation for an address that isn't on any ACL. See [Switching between sites](#switching-between-sites).
  - **`lastPostAt` is declared but nothing keeps it current.** It is written when a membership is created and never touched again, so the "most recently updated first" ordering degrades to "most recently joined first". Correcting it means fanning out a write to every member row of a slug on every new post, and the partition key is the *email* — so updating by slug is a cross-partition operation on the ingest hot path. That is the wrong trade for a list that currently has one entry. **The alternative worth considering before building the fan-out** is a per-slug row holding `lastPostAt` once, joined at read time against a membership list that is only ever a handful of rows.
  - **The root half shipped, and it lists rather than redirects.** A signed-in visitor is offered their archives by name instead of being bounced to the most recent one. Redirecting would have been fewer clicks and worse: someone signed in with the wrong account — the work address rather than the personal one — needs to be able to *see* that, and a page that jumps straight through gives them no opportunity to notice. The no-memberships case is stated plainly for the same reason, since a signed-in person staring at a page that looks like it should have their letters on it will conclude they have lost them. The in-site switcher is still to come.
...
4. **`mailauth` carries four high-severity advisories with no fix available.** It depends on vulnerable `nodemailer` and `undici`, and `npm audit` has been reporting this since before the claim work — it was surfaced by adding `@azure/data-tables`, not caused by it. The exposure looks small: the `nodemailer` advisories are all about *sending* mail, and nothing here sends through `nodemailer`. It is not zero, because `undici` is a live HTTP client and `mailauth` is fed attacker-controlled bytes on every inbound message. **The question is whether DKIM verification needs `mailauth` at all**, or whether the one function used — verifying an embedded original's signature — is worth pulling in a mail-sending library for. Deciding it before the pilot is cheap; deciding it after a CVE lands is not.
