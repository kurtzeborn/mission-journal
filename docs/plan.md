...
- **Owner-only, capped at 24 per letter, 20 MB per picture — and ~~`ETag`-guarded~~ deliberately not.** Every other write here replaces a document the browser composed from a copy of the site, which is why they carry the staleness claim. These two do not: one appends an entry whose id is the hash of the bytes, the other names a single id to drop. Neither can undo an edit it never saw, and enforcing it would break the ordinary case of adding two pictures in a row — the first moves the `ETag` the second was holding. The cap counts only what an owner added, so a forward with thirty attachments does not lock its own owner out.
