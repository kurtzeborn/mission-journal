...
5. **Claim.** The link opens `pdayletters.com/claim#{token}`, and that page is **anonymous-allowed**: it leads with what is being claimed — the missionary's name, the counts, up to three subject lines and dates — then a plain statement of what happens next, then the **Sign in with Google / Sign in with Microsoft** pair.
   ...
   **Expired and already-used links.** The token shares the pending site's rolling `expiresAt`, so it lives exactly as long as there is something to claim. The failure page still does something useful: if the site is now active, *"This site is already set up"* plus a sign-in link; if the pending site has purged, say so plainly; if the token is merely stale or spent, offer **"Email me a new link"**, which sends only to an address already listed in `claim.json`. *(That last offer is not built.)*
...
**What remains:** the recurring reminder and the tapering invitation series, and the "email me a new link" path for an expired token. The claim email itself, the `claim@` handler and the "a site already exists" reply all shipped with [Phase 8](#phase-8--outbound-mail-and-preferences).
...
- **Pending sites and claim** per [Onboarding and auto-provisioning](#onboarding-and-auto-provisioning). Shipped: the anonymous landing page showing sender, waiting count and sample subjects before sign-in; the first owner; the display name; promotion of accumulated raw; the failure page; the rolling `expiresAt` and the timer-triggered purge. **The sample subjects and the count are recorded on `claim.json` as letters arrive**, rather than computed by opening every held message on a page load, which would time out for exactly the families with the most to lose. Capped at three, because a stolen link should not hand over a summary of the whole archive. Still missing: the reminder, the tapering series, and "email me a new link".
...
- **Ingest nudges**, in `nudge.js` — the reply sent when a bootstrap forward fails DMARC, offering the relay link second. This is a single reply, not the recurring reminder series Phase 7 still wants.
