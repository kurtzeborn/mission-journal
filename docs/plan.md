...
- **`users`** — `PartitionKey = "user"`, `RowKey = lowercased email address`. Identity columns: `displayName`, `authProvider` (`google` | `microsoft`), `firstSeenAt`, `lastSignInAt`. Preference columns: `postAckEmails` and `dedupeAckEmails` (bools — see [Notification preferences](#notification-preferences) for the per-sender-type defaults); additional per-user preferences are just additional columns as they arrive. State columns: `claimEmailSentAt`, the timestamp of the one-time claim invitation sent to a missionary who bootstrapped their own pending site — kept here rather than in `claim.json` so the guarantee survives a pending-site purge and recreation.
...
7. **Reminder and purge.** Seven days after the last claim email, one reminder goes to the forwarder addresses already emailed. The missionary is never reminded — their invitation is one-time by design. When `expiresAt` passes with no intervening message, a timer-triggered Function purges `pending/{slug}/` entirely, including soft-deleted blob versions.
...
This is why **pending-site claim emails count as replies, not as self-originated mail.** Each one answers a specific inbound message, so it gets the `From:` and threading treatment above. That matters most for the one-time invitation sent to a missionary who bootstrapped their own site (see [Onboarding and auto-provisioning](#onboarding-and-auto-provisioning)) — it lands in a Gmail inbox at `missionary.org`, carries a claim link, and needs every deliverability signal available to it.
...
- **Pending sites and the claim flow** per [Onboarding and auto-provisioning](#onboarding-and-auto-provisioning): unresolvable-but-valid slugs create `pending/{slug}/`; a claim email to the forwarder, or the **one-time** invitation to a missionary whose own `direct` message created the site, guarded by `claimEmailSentAt` on their `users` row; `/claim/{token}` endpoint that establishes the first owner and promotes accumulated raw; day-7 reminder to forwarders only; rolling `expiresAt` reset on every message, at 60 days once `hasDirect` is set and 14 days otherwise; timer-triggered purge when the window lapses.
...
- **Verification:** hand-craft duplicate forwards from a test mailbox and confirm the raw folder count stays flat while the ack arrives. Bulk-forward five near-simultaneously to exercise the ETag-retry path. Forward from an unknown address, confirm a pending site is created and nothing renders, then claim it and confirm all accumulated messages appear deduplicated and in date order. Let a second pending site expire and confirm it purges. Send a `direct` message from the stand-in missionary domain to a slug with no site and confirm exactly one invitation goes out, that a second `direct` message resets `expiresAt` without sending another, and that the window is 60 days rather than 14.
