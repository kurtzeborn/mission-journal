...
- **[Restoring a post to its original](#restoring-the-original) discards them, and that is settled.** Restore re-renders from `raw/`, and a photo the owner added was never in `raw/` — so it falls out for free, with no code and no second stored state distinguishing an added photo from a mailed one. Asking whether to keep them was rejected on those grounds. The confirmation already says so out loud, before any of this is built, so the answer cannot quietly become whatever the code happens to do.
