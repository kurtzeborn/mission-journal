...
- **The account block was a false alarm, and an expensive one.** Peecho's Company details form demands a VAT number a US sole trader does not have, and the API answered `APP_NO_COMP_DETAILS`, so this was written down as blocked. It is not: the API wants **Billing Information and Billing Address**, neither of which asks for VAT, and once those were filled in a create-publication call returned a real secure publication id and a working checkout page. Recorded in [printing.md](printing.md) so the next person does not spend an evening on a form nothing reads. `PEECHO_API_KEY` is still unset in every environment and the order endpoint still answers `503 printing is not switched on yet`, but now only because there is nothing to press yet.

**Next:** the order button on `/book/{slug}`, and two things that first call exposed — `PEECHO_OFFERING_ID`, without which the buyer is asked to choose between BOOKS and MAGAZINES before seeing their own book, and `order.product.thumbnail`, whose absence leaves the configurator with no preview and a `thumbnailUrl Required` in its console. Then the line in pdayletters.com's own terms backing the IP and portrait-rights warranty Peecho's seller terms require.
