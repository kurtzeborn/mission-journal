...
**In progress.** The layout engine, the pipeline that drives it, and the provider integration are built; the provider account is blocked on a form we cannot fill.
...
- **Test environment by default.** `PEECHO_BASE` is `https://test.www.peecho.com` unless production is named explicitly, because the cost of getting that default the wrong way round is a real book, really printed, really shipped.
- **Still blocked, and not by us.** Peecho's company-details form requires a VIES-validated VAT number that a US sole trader does not have; without it the API answers `APP_NO_COMP_DETAILS` and the webhook settings will not save. So `PEECHO_API_KEY` is unset in every environment and the order endpoint answers `503 printing is not switched on yet` rather than pretending.

**Next:** the order button on `/book/{slug}`, which is deliberately not shipped while pressing it can only produce a 503; and the line in pdayletters.com's own terms backing the IP and portrait-rights warranty Peecho's seller terms require.
