...
**Partly shipped.** The classifier's `direct` branch is implemented and returns `publish`; the corpus carries three direct captures — `direct-missionary`, `direct-bcc-inline-via-cloudflare`, `direct-bcc-inline-via-exchange` — and a real `@missionary.org` send is on record passing DMARC at our edge. The `originalDate` ordering is fixed and covered, warning-level logging of a missionary-domain rejection is built, and an unclaimed direct send is held as a pending site. **What is genuinely outstanding: one live end-to-end run.**

**Gated on access to a real `@missionary.org` account.** See [Building blind](#building-blind).
