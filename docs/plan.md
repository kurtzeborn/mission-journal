...
- ~~**Suppression has to be visible to owners, not just to us.**~~ **Shipped 2026-08-06**, as `delivery.js` and the `deliveries` table, keyed by SHA-256 of the address exactly as `optouts` is.
  ...
  - **On the page it dims rather than badges**, the same 0.55 as a hidden letter, with the reason on the line in ordinary words. **The controls stay at full contrast** — Resend is often the next thing to press, and dimming the remedy along with the problem would be the wrong half. The colour is warm rather than red, because an alarming colour beside a grandmother's address reads as an accusation.
