...
- **It carries the one-click opt-out** already built for acks, which here means "never invite me to anything again" and is honored ahead of any future invitation to that address. **Not shipped, because the opt-out it refers to does not exist yet** — it is Phase 8's, still to build. This is the one outstanding piece that leaves a stranger able to be mailed with no way to stop it, and it should be built alongside the opt-out rather than after it.
...
Binding on acceptance avoids that entirely: whatever identity signs in *through the invitation link* is written to the ACL. The typed address is **where to send the invitation**, not **who the person must prove they are**. Both are recorded — `invitedEmail` alongside the bound identity — so the owner's list still shows the address they typed. **Only half shipped: the ACL entry records `invitedBy`, not `invitedEmail`.** So an owner who invited `grandma@aol.com` sees `g.example@gmail.com` appear on the list with nothing connecting the two — which is the same confusion this mechanism exists to prevent, moved off the invitee and onto the owner. Outstanding, and small.
...
**Partly shipped, out of order.** Edit, hide and delete, the `/login` chooser, the `401` deep link, the signed-in state on the root page, the `memberships` index, the root archive list, and now **invitations and member management** were all pulled forward — each because something already built was resting on it. **Everything else is untouched:** no site switcher inside a site, no `403` page, no restore-original, no site deletion, no profile editing, no operator role.
...
- **Invitations** per [Invitations](#invitations): bulk paste-and-parse, one signed single-use email per invitee naming the inviting owner, identity binding on acceptance rather than address matching, `invited` / `active` state in the admin list, and manual owner-initiated resend that invalidates the prior token. **Capped per site and per day**, since an uncapped bulk field is an outbound-mail cannon pointed at the sending domain's reputation.
  ...
  - **Still outstanding:** bulk paste-and-parse, owner-initiated resend, `invitedEmail` on the ACL entry, and the one-click opt-out an invitation is supposed to carry.
