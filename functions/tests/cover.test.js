...
describe('what a cover is bound in', () => {
    test('gives every color an ink that is not the same color', () => {
    ...
    test('binds an archive that has never been asked', () => {
        assert.deepEqual(coverOf({}), { cloth: DEFAULT_CLOTH, picture: '' });
    });

    test('does not bind anything in white', () => {
    ...
    test('falls back rather than refusing when a stored choice has drifted', () => {
    ...
    test('keeps a picture that looks like one of ours', () => {
...
describe('choosing a cover', () => {
    ...
    test('refuses a picture that is not one of ours', () => {
        assert.ok(chooseCover({ cloth: 'navy', picture: '../../secrets' }).error);
    });
});

describe('remembering a cover', () => {
    test('keeps everything else on the profile', async () => {
    ...
    test('writes a profile for an archive that has never had one', async () => {
        const store = memoryStore();
    ...
    test('refuses when somebody else saved first', async () => {
        const store = memoryStore();
...
describe('the picture on the front board', () => {
    test('reads nothing when no picture was chosen', async () => {
        const store = memoryStore();
        assert.equal(await readCoverPicture({ store, slug: 'isaac.backman', cover: {} }), null);
    });

    test('reads a photograph out of the archive', async () => {
        const store = memoryStore();
        await store.writeBlob(
            'rendered',
    ...
    test('prints the cloth alone when the photograph has since been deleted', async () => {
    ...
    test('stores an upload as a stripped rendition of its own', async () => {
        const store = memoryStore();
    ...
    test('reads an upload back', async () => {
        const store = memoryStore();
    ...
    test('replaces the last upload rather than keeping both', async () => {
