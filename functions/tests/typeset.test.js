...
describe('cutting a paragraph into pieces a line may end on', () => {
    test('keeps the space that followed a word on the word', () => {
        assert.deepEqual(texts(segments([{ text: 'one two three' }])), ['one ', 'two ', 'three']);
    });

    test('remembers the word without its space, for when it ends a line', () => {
        assert.deepEqual(
    ...
    test('keeps two line breaks as two, so the blank line survives', () => {
        assert.equal(segments([{ text: 'up\n\ndown' }]).filter((piece) => piece.hard).length, 2);
    });

    test('breaks a paragraph that opens on one', () => {
        assert.deepEqual(texts(segments([{ text: '\ndown' }])), ['', 'down']);
    });

    test('carries each run its own styling', () => {
...
describe('filling one line', () => {
    test('takes as many pieces as the width allows', () => {
        const pieces = segments([{ text: 'one two three four' }]);

        assert.deepEqual(texts(line(pieces, 0, 80)), ['one ', 'two ']);
    });

    test('does not pay for the space it will never print', () => {
    ...
    test('carries on from where the line before stopped, at whatever width it is given', () => {
